...
describe("Checkbox Component", () => {
  ...
  test("tidak boleh memanggil `onChange` jika dalam keadaan disabled", () => {
    const handleChange = jest.fn();
    render(
      <Checkbox onChange={handleChange} disabled data-testid="my-checkbox" />
    );
...
const dbMock = {
  asset: {
    findMany: jest
      .fn()
      .mockResolvedValue([{ id: "AST-INT-001", name: "Integration Asset" }]),
  },
};
...
describe("Alur Login Pengguna", () => {
  it("harus berhasil login dengan kredensial yang valid dan diarahkan ke dashboard", () => {
    ...
    cy.get('input[name="email"]')
      .should("be.visible")
      .type("super.admin@trinitymedia.co.id");
  ...
  it("harus menampilkan pesan error dengan kredensial yang tidak valid", () => {
    ...
    cy.get('input[name="email"]').type("email.salah@trinitymedia.co.id");
    cy.get('input[name="password"]').type("password.salah");
    cy.contains("button", "Masuk").click();

    // Verifikasi bahwa pesan error muncul
    cy.contains("Email atau kata sandi yang Anda masukkan salah").should(
      "be.visible"
    );
