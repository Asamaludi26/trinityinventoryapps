...
- **Solusi**:
  - Pastikan backend menggunakan `prisma.$transaction` dengan teknik _row locking_ atau validasi stok ulang sesaat sebelum `update`.
  - Lakukan _Stock Opname_ (penyesuaian manual) untuk mengoreksi angka di sistem sesuai fisik.
...
- **Solusi**:
  - Edit konfigurasi nginx (`/etc/nginx/sites-available/trinity-asset`).
