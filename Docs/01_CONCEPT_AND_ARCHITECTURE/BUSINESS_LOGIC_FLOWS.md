...
classDiagram
    ...
    class IHandoverStrategy {
        <<interface>>
        +handle(data)
        +validate(data)
    }
    class LoanStrategy {
        +handle() : Mengurangi stok sementara
        +validate() : Cek batas waktu peminjaman
    }
    class DismantleStrategy {
        +handle() : Menambah stok gudang (Return)
        +validate() : Cek kondisi aset
    }
    class InstallationStrategy {
        +handle() : Mengurangi stok permanen -> Aset Pelanggan
        +validate() : Cek kelengkapan dokumen
    }

    HandoverContext --> IHandoverStrategy
    IHandoverStrategy <|-- LoanStrategy
    IHandoverStrategy <|-- DismantleStrategy
    IHandoverStrategy <|-- InstallationStrategy
```
