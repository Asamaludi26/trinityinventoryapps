...
const CheckListItem: React.FC<{ met: boolean; text: React.ReactNode }> = ({
  met,
  text,
}) => (
  <li
    className={`flex items-start gap-2 transition-colors ${met ? "text-green-700" : "text-gray-500"}`}
  >
    {met ? (
  ...
  return (
    <FormPageLayout title="Kelola Akun Saya">
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto space-y-8">
        <FormSection
          title="Profil"
          icon={<UserIcon className="w-6 h-6 mr-3 text-primary-600" />}
        >
          <div>
            <label
              htmlFor="name"
            ...
            <input
              ...
              required
              className="block w-full px-3 py-2 mt-1 bg-gray-50 border border-gray-300 text-sm text-gray-700 rounded-lg shadow-sm"
            />
            {nameError && (
              <p className="mt-1 text-xs text-danger-text">{nameError}</p>
            )}
          </div>
          <div>
            <label
            ...
            />
            {emailError && (
              <p className="mt-1 text-xs text-danger-text">{emailError}</p>
            )}
          </div>
        </FormSection>
        ...
        >
          ...
          <div>
            ...
            <div className="relative mt-1">
              <input
                ...
                className="block w-full px-3 py-2 pr-10 bg-gray-50 border border-gray-300 text-sm text-gray-700 rounded-lg shadow-sm"
              />
              <button
                type="button"
                onClick={() =>
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <button
                type="button"
                onClick={() =>
                  setPasswordVisibility((p) => ({ ...p, new: !p.new }))
                }
                className="absolute inset-y-0 right-0 flex items-center px-3 text-gray-500 hover:text-primary-600"
              >
          ...
          {passwordError && (
            <p className="mt-1 text-xs text-danger-text">{passwordError}</p>
          )}
        </FormSection>
