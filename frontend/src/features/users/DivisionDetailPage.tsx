...
const DivisionDetailPage: React.FC<DivisionDetailPageProps> = ({
  pageInitialState,
  onBack,
  onEdit,
  onViewMember,
  ...
  const totalAssetsInDivision = storeAssets.filter((asset) =>
    members.some((member) => member.name === asset.currentUser),
  ).length;
  const leaderCount = members.filter(
    (member) => member.role === "Leader",
  ).length;

  // SECURITY CHECK: Admin Logistik restriction logic
  const isRestrictedView = currentUser?.role === "Admin Logistik";
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {members.length > 0 ? (
                    members.map((member) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600">
                          {isRestrictedView && member.id !== currentUser?.id ? (
                        ...
                        <td className="px-4 py-3">
                          {isRestrictedView && member.id !== currentUser?.id ? (
                            <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-gray-100 text-gray-400">
                              Tersembunyi
                            </span>
                          ) : (
                            <span
                              className={`px-2 py-0.5 text-xs font-semibold rounded-full ${getRoleClass(member.role)}`}
                            >
                              {member.role}
                            </span>
                          )}
                    ...
                    <tr>
                      ...
                      >
                        Divisi ini tidak memiliki anggota.
                      </td>
                    </tr>
                  )}
                </tbody>
