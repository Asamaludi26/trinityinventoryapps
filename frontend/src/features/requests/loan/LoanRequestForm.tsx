...
interface LoanRequestFormProps {
  ...
  onSave: (data: { loanItems: LoanItem[]; notes: string }) => Promise<void>;
  onCancel: () => void;
  currentUser: User;
  divisions: Division[];
}
...
export const LoanRequestForm: React.FC<LoanRequestFormProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const openItemIndex = loanItems.findIndex(
        (item) => item.tempId === openDropdownId,
      );
        ...
        if (item.tempId === tempId) {
          if (field === "quantity") {
            const selectedModel = availableModels.find(
              (m) => `${m.name}|${m.brand}` === item.modelKey,
            );
            ...
            if (value === "") {
              ...
              if (value > maxStock) {
                finalValue = maxStock;
                addNotification(
                  `Jumlah tidak bisa melebihi stok yang tersedia (${maxStock}).`,
                  "warning",
                );
              } else if (value < 1) {
                finalValue = 1;
  ...
  const handleApplyToSelected = () => {
    ...
    setSelectedItemIds([]);
    addNotification(
      "Tanggal pengembalian berhasil diterapkan ke item terpilih.",
      "success",
    );
  };
  ...
  const isSubmitDisabled = useMemo(() => {
    ...
    const hasInvalidItems = loanItems.some(
      (item) => !item.modelKey || item.quantity === "" || item.quantity <= 0,
    );
    if (hasInvalidItems) return true;

    const hasInvalidDates = loanItems.some(
      (item) => !item.isIndefinite && !item.returnDate,
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSubmitDisabled) {
      addNotification(
        "Harap lengkapi semua informasi yang diperlukan, termasuk tanggal kembali untuk setiap item.",
        "error",
      );
      return;
    }
      ...
      <form id={formId} onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="text-center sm:text-left md:text-center border-b pb-4 sm:border-none sm:pb-0">
          <h3 className="text-lg sm:text-xl font-bold uppercase text-gray-900">
            Surat Permintaan Peminjaman Aset
          </h3>
          <p className="text-sm text-gray-500">Nomor: [Otomatis]</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-x-8 sm:gap-y-4 md:grid-cols-2 text-sm bg-gray-50/50 p-4 rounded-lg border border-gray-100">
          <div>
            ...
            <input
              ...
              className="w-full p-2 mt-1 bg-white border border-gray-200 rounded-md text-gray-600 shadow-sm"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block font-medium text-gray-700">Divisi</label>
            <input
              type="text"
              readOnly
              value={userDivision}
        ...
        <section className="pt-4 border-t">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
            ...
            <button
              ...
              className="w-full sm:w-auto px-4 py-2 text-sm font-semibold text-white transition-colors bg-primary-500 rounded-lg shadow-sm hover:bg-primary-600"
            >
              + Tambah Aset
            </button>
          </div>
          ...
          {loanItems.length > 1 && (
            <div className="p-4 border rounded-lg bg-blue-50/30 border-blue-200/80 mb-6">
              <h5 className="font-semibold text-primary-600 mb-3 text-sm">
                Aksi Massal
              </h5>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-end pb-2">
                  <label
                    htmlFor="isGlobalIndefinite"
                    className="flex items-center space-x-3 cursor-pointer text-sm text-gray-700 bg-white px-3 py-2 rounded-md border border-gray-200 w-full shadow-sm"
                  >
                    <Checkbox
                      id="isGlobalIndefinite"
                      checked={isGlobalIndefinite}
                      onChange={(e) => setIsGlobalIndefinite(e.target.checked)}
                    />
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 text-sm font-semibold text-white bg-primary-600 rounded-lg shadow-sm hover:bg-primary-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
            ...
            <div className="flex items-center space-x-3 mb-4 p-2 border-b bg-gray-50/50 rounded-t-lg">
              <Checkbox
                ...
                checked={
                  ...
                  selectedItemIds.length === loanItems.length
                }
          ...
          <div className="space-y-6">
            {loanItems.map((item, index) => {
              ...
              const filteredOptions = getFilteredOptions(
                item.modelKey,
                itemInputs[item.tempId] || "",
              );
              return (
                <div
                  key={item.tempId}
                  className={`relative p-4 sm:p-5 pt-6 bg-white border rounded-xl shadow-sm transition-all duration-300 ${selectedItemIds.includes(item.tempId) ? "border-primary-600 ring-2 ring-primary-500/30" : "border-gray-200/80"}`}
                >
                  <div className="absolute flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 font-bold text-white rounded-full -top-3.5 left-3 sm:-top-4 sm:left-4 bg-primary-600 shadow-lg text-sm">
                    {index + 1}
                  </div>
                  ...
                  {loanItems.length > 1 && (
                    <button
                      ...
                      className="absolute top-2 right-2 sm:top-3 sm:right-3 flex items-center justify-center w-8 h-8 text-gray-400 transition-colors rounded-full hover:bg-red-100 hover:text-red-500 z-10"
                  ...
                  {loanItems.length > 1 && (
                    <div className="flex items-center space-x-3 mb-4 pl-1">
                      ...
                      />
                      <label
                        htmlFor={`select-item-${item.tempId}`}
                        className="text-sm font-medium text-gray-700 cursor-pointer"
                      >
                        Pilih item ini
                      </label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    ...
                    >
                      <label className="block text-sm font-medium text-gray-700">
                        Pilih Aset <span className="text-danger">*</span>
                      </label>
                      <div className="relative mt-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleModelInputChange(item.tempId, e.target.value)
                          }
                          onClick={() =>
                            setOpenDropdownId(
                              item.tempId === openDropdownId
                                ? null
                                : item.tempId,
                            )
                          }
                          placeholder="Cari nama aset..."
                          className="block w-full py-2 pl-10 pr-4 text-sm text-gray-900 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-primary-500"
                        />
                              ...
                              {filteredOptions.length > 0 ? (
                                filteredOptions.map((opt) => (
                                  <li
                                    key={opt.value}
                                    onClick={() =>
                                      handleModelSelect(item.tempId, opt.value)
                                    }
                                    className="px-4 py-2.5 text-sm text-gray-800 cursor-pointer hover:bg-blue-50 hover:text-primary-600 flex items-center justify-between"
                                  >
                                    <span>{opt.label}</span>
                                    <div className="flex items-center gap-2">
                                      {opt.isMaterial ? (
                                        ...
                                        <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded flex items-center gap-1">
                                          <BsBoxSeam className="w-2.5 h-2.5" />{" "}
                                          Unit
                                        </span>
                                      )}
                                      <span className="text-xs text-gray-500 font-semibold bg-gray-100 px-1.5 py-0.5 rounded">
                                        Stok: {opt.count}
                                      </span>
                                    </div>
                                  </li>
                    ...
                    <div className="grid grid-cols-2 sm:col-span-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="block w-full px-3 py-2 mt-1 text-center text-gray-700 bg-gray-100 border border-gray-300 rounded-lg shadow-sm"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">
                        ...
                        <input
                          ...
                          className="block w-full px-3 py-2 mt-1 text-center text-gray-900 bg-white border border-gray-300 rounded-lg shadow-sm disabled:bg-gray-100 disabled:text-gray-500"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="mt-3">
                    ...
                    <input
                      ...
                      placeholder="Kebutuhan spesifik..."
                      className="block w-full px-3 py-2 mt-1 text-sm text-gray-900 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-primary-500"
                    />
                  </div>
                  <div className="pt-4 mt-4 border-t">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700">
                          Tgl Kembali <span className="text-danger">*</span>
                        </label>
                        <div className="mt-1">
                          <DatePicker
                            id={`rd-${item.tempId}`}
                            selectedDate={item.returnDate}
                            onDateChange={(date) =>
                              handleItemChange(item.tempId, "returnDate", date)
                            }
                            disablePastDates
                            disabled={item.isIndefinite}
                          />
                        </div>
                      </div>
                      <div className="flex items-end pb-1">
                        <label
                          htmlFor={`indef-${item.tempId}`}
                          className="flex items-center space-x-3 cursor-pointer text-sm text-gray-700 bg-gray-50 px-3 py-2 rounded-md border border-gray-200 w-full"
                        >
                          <Checkbox
                            id={`indef-${item.tempId}`}
                            checked={item.isIndefinite}
                            onChange={(e) =>
                              handleItemChange(
                                item.tempId,
                                "isIndefinite",
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <textarea
                ...
                rows={4}
                className="w-full pl-10 pr-4 py-2 border-gray-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500 bg-gray-50 text-gray-700 placeholder:text-gray-400"
        ...
        <section className="pt-8 mt-6 border-t border-gray-200">
          <div className="grid grid-cols-1 text-sm text-center gap-y-8 md:grid-cols-2 md:gap-x-8">
            <div>
              ...
              <p className="pt-1 mt-2 border-t border-gray-400 text-gray-600">
                {currentUser.name}
              </p>
            </div>
            <div>
              <p className="font-semibold text-gray-700">
                Mengetahui (Admin Logistik),
              </p>
              <div className="flex items-center justify-center mt-2 h-28">
