...
const ReturnAssetFormPage: React.FC<ReturnAssetFormPageProps> = ({
  ...
  const submitReturnRequest = useRequestStore(
    (state) => state.submitReturnRequest,
  );
  ...
  const [returnDetails, setReturnDetails] = useState<
    Record<string, { condition: AssetCondition; notes: string }>
  >({});
  ...
  useEffect(() => {
    const initialDetails: Record<
      string,
      { condition: AssetCondition; notes: string }
    > = {};
  ...
  const getDivisionForUser = (userName: string): string => {
    ...
    return user && user.divisionId
      ? `Divisi ${divisions.find((d) => d.id === user.divisionId)?.name || ""}`
      : "";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!loanRequest || selectedAssetIds.length === 0) {
      addNotification(
        "Pilih setidaknya satu aset untuk dikembalikan.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const returnItemsPayload = selectedAssetIds.map((assetId) => ({
        assetId,
        condition:
          returnDetails[assetId]?.condition || AssetCondition.USED_OKAY,
        notes: returnDetails[assetId]?.notes || "",
      }));

      await submitReturnRequest(loanRequest.id, returnItemsPayload);
      addNotification(
        `Pengajuan pengembalian berhasil dibuat (Dokumen: ${docNumber}).`,
        "success",
      );
    } catch (error: any) {
      addNotification(
        error.message || "Gagal mengajukan pengembalian.",
        "error",
      );
    } finally {
  ...
  const ActionButtons: React.FC<{ formId: string }> = ({ formId }) => (
    <>
      <button
        type="button"
        onClick={onCancel}
        className="px-5 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50"
      >
        Batal
      </button>
      <button
        ...
        <form id={formId} onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="p-4 bg-gray-50 border rounded-lg text-sm grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                className="w-full mt-1 p-2 bg-gray-100 border rounded text-gray-700 font-mono"
                value={docNumber}
                readOnly
              />
            </div>
            <div>
              ...
              <input
                className="w-full mt-1 p-2 bg-gray-100 border rounded text-gray-700"
            ...
            <div className="space-y-3">
              {preselectedAssets.map((asset) => {
                ...
                return (
                  <div
                    key={asset.id}
                    className={`p-4 border rounded-lg transition-all duration-300 ${isSelected ? "border-primary-600 bg-blue-50/30" : "border-gray-200 bg-white"}`}
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 font-mono">
                          {asset.id} / SN: {asset.serialNumber || "N/A"}
                        </p>

                        <div
                          className={`mt-3 space-y-3 transition-all duration-300 ease-in-out overflow-hidden ${isSelected ? "max-h-40 opacity-100 pt-3 border-t" : "max-h-0 opacity-0"}`}
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                              ...
                              <CustomSelect
                                options={Object.values(AssetCondition).map(
                                  (c) => ({ value: c, label: c }),
                                )}
                                value={
                                  returnDetails[asset.id]?.condition ||
              ...
              <div>
                ...
                <div className="flex items-center justify-center mt-2 h-28">
                  <SignatureStamp
                    ...
                    signerDivision={getDivisionForUser(currentUser.name)}
                  />
                </div>
