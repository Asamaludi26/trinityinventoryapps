...
const ReturnRequestDetailPage: React.FC<ReturnRequestDetailPageProps> = (
  props,
) => {
  const {
    returnDocuments,
  ...
  const processReturnBatch = useRequestStore(
    (state) => state.processReturnBatch,
  );
  ...
  const loanRequest = useMemo(
    () =>
      storeLoanRequests.find((lr) => lr.id === returnDocument?.loanRequestId) ||
      props.loanRequest,
  ...
  if (!returnDocument) {
    return (
      <div className="p-8 text-center text-gray-500">
        Data dokumen pengembalian tidak ditemukan atau telah dihapus.{" "}
        <button
  ...
  const getDivisionForUser = (userName: string): string => {
    ...
    return user && user.divisionId
      ? divisions.find((d) => d.id === user.divisionId)?.name || ""
      : "";
  };
  ...
  const handleRejectAll = async () => {
    if (
      !window.confirm(
        "Apakah Anda yakin ingin menolak SELURUH item dalam dokumen ini? Status aset akan tetap 'Dipinjam'.",
      )
    )
      return;
    setIsLoading(true);
  ...
  const handleVerificationConfirm = async (acceptedAssetIds: string[]) => {
    if (
      acceptedAssetIds.length === 0 &&
      !window.confirm(
        "Anda tidak memilih aset apapun. Ini akan dianggap sebagai PENOLAKAN TOTAL. Lanjutkan?",
      )
    ) {
      return;
    }
  ...
  return (
    ...
    >
      <div className="space-y-8">
        <div
          ref={printRef}
          className="p-8 bg-white border border-gray-200/80 rounded-xl shadow-sm space-y-8"
        >
          <Letterhead />
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500">
              Nomor: {returnDocument.docNumber}
            </p>
          </div>
            ...
            <dl className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 text-sm">
              <div>
                ...
                <dd className="font-semibold text-gray-900">
                  {new Date(returnDocument.returnDate).toLocaleDateString(
                    "id-ID",
                    ...
                    <ClickableLink
                      onClick={() =>
                        setActivePage("request-pinjam", {
                          openDetailForId: loanRequest.id,
                        })
                      }
                    >
                      {loanRequest.id}
              ...
              <div>
                <dt className="font-medium text-gray-500">Dikembalikan Oleh</dt>
                <dd className="font-semibold text-gray-900">
                  {returnDocument.returnedBy}
                </dd>
              </div>
              <div>
                <dt className="font-medium text-gray-500">Divisi</dt>
                <dd className="font-semibold text-gray-900">
                  {getDivisionForUser(returnDocument.returnedBy)}
                </dd>
              </div>
            </dl>
          </section>
                    ...
                    return (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="p-3 font-semibold text-gray-800">
                          <ClickableLink
                            onClick={() =>
                              onShowPreview({ type: "asset", id: item.assetId })
                            }
                          >
                            {item.assetName}
                          </ClickableLink>
                        ...
                        <td className="p-3">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${statusBadgeClass}`}
                          >
                            {statusLabel}
                          </span>
                        </td>
            ...
            <div className="grid grid-cols-1 text-sm text-center gap-y-6 sm:grid-cols-2">
              <div>
                ...
                <div className="flex items-center justify-center mt-2 h-28">
                  <SignatureStamp
                    signerName={returnDocument.returnedBy}
                    signatureDate={returnDocument.returnDate}
                    signerDivision={getDivisionForUser(
                      returnDocument.returnedBy,
                    )}
                  />
                </div>
              ...
              <div>
                <p className="font-semibold text-gray-600">
                  Diterima (Admin Logistik),
                </p>
                <div className="flex items-center justify-center mt-2 h-28">
                  {returnDocument.status === AssetReturnStatus.REJECTED &&
                    returnDocument.verifiedBy && (
                      <RejectionStamp
                        rejectorName={returnDocument.verifiedBy}
                        rejectionDate={returnDocument.verificationDate!}
                      />
                    )}
                  {(returnDocument.status === AssetReturnStatus.COMPLETED ||
                    ...
                    returnDocument.verifiedBy && (
                      <ApprovalStamp
                        approverName={returnDocument.verifiedBy}
                        approvalDate={returnDocument.verificationDate!}
                      />
                    )}
                  {returnDocument.status ===
        ...
        {isVerificationPanelOpen &&
          returnDocument.status === AssetReturnStatus.PENDING_APPROVAL && (
            <div ref={panelRef}>
