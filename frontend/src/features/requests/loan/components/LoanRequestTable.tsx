  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className="group-hover:text-slate-900 transition-colors whitespace-nowrap">
          {children}
        </span>
        <span className="flex-shrink-0 flex items-center justify-center">
          {isSorted ? (
            <span className="text-primary-600 bg-blue-100/50 p-0.5 rounded shadow-sm animate-fade-in-up">
              {direction === "ascending" ? (
...
export const LoanRequestTable: React.FC<LoanRequestTableProps> = ({
  requests,
  onDetailClick,
  sortConfig,
  requestSort,
    ...
    <table className="min-w-full divide-y divide-slate-100">
      <thead className="sticky top-0 z-10 bg-slate-50/95 backdrop-blur-sm border-b border-slate-200 shadow-sm">
        <tr>
          ...
          <th
            scope="col"
            className="px-4 sm:px-6 py-4 text-xs sm:text-sm font-extrabold uppercase tracking-wider text-slate-700 text-left"
          >
            Detail Barang
          </th>
          <SortableHeader
          ...
          <th className="relative px-4 sm:px-6 py-4 w-24 text-right text-xs sm:text-sm font-extrabold uppercase tracking-wider text-slate-700">
            Aksi
          </th>
        </tr>
      </thead>
        ...
        {requests.length > 0 ? (
          requests.map((req) => {
            ...
            if (isHighlighted) {
              ...
              bgClass =
                "hover:bg-red-50/30 border-l-transparent hover:border-l-red-400";
            } else {
              bgClass =
              ...
              >
                ...
                <td className="px-4 sm:px-6 py-4 sm:py-5 align-top">
                  <div className="flex flex-col gap-1.5">
                    <span className="text-sm font-bold text-slate-800 group-hover:text-primary-600 transition-colors tracking-tight whitespace-nowrap">
                      {req.id}
                    </span>
                ...
                <td className="px-4 sm:px-6 py-4 sm:py-5 align-middle">
                  <div className="flex flex-col">
                    <div
                      className="text-sm font-bold text-slate-800 truncate max-w-[120px] sm:max-w-none"
                      title={req.requester}
                    >
                      {req.requester}
                    </div>
                    <div className="text-xs font-medium text-slate-500 mt-0.5 truncate max-w-[120px] sm:max-w-none">
                      {req.division}
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 sm:px-6 py-4 sm:py-5 align-middle">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-shrink-0 bg-slate-100 text-slate-600 font-bold px-2 py-1.5 rounded-lg text-xs border border-slate-200 shadow-sm flex flex-col items-center min-w-[2.5rem]">
                      <span className="text-base sm:text-lg leading-none tracking-tight">
                        {req.items.length}
                      </span>
                    ...
                    <div className="flex flex-col justify-center min-w-0">
                      <div
                        className="text-sm font-semibold text-slate-800 line-clamp-1 max-w-[140px] sm:max-w-[180px]"
                ...
                <td className="px-4 sm:px-6 py-4 sm:py-5 align-middle">
                  <span
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] sm:text-[11px] font-bold uppercase tracking-wider rounded-full shadow-sm whitespace-nowrap ${getStatusClass(req.status)}`}
                  >
                    {req.status}
                  </span>
                </td>
            ...
            <td colSpan={5} className="px-6 py-20 text-center">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-slate-500 mt-1 max-w-xs mx-auto">
                  Belum ada data request peminjaman yang sesuai dengan filter
                  Anda.
                </p>
              </div>
            </td>
