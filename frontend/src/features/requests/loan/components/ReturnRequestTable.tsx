    ...
    <table className="min-w-full divide-y divide-slate-100">
      <thead className="sticky top-0 z-10 bg-slate-50/95 backdrop-blur-sm border-b border-slate-200 shadow-sm">
        <tr>
          ...
          <th
            scope="col"
            className="px-6 py-4 text-sm font-extrabold uppercase tracking-wider text-slate-700 text-left"
          >
            Ringkasan Item
          </th>
          <SortableHeader
            columnKey="returnedBy"
            sortConfig={sortConfig}
            requestSort={requestSort}
          >
            Pengembali
          ...
          <th className="relative px-6 py-4 w-24 text-right text-sm font-extrabold uppercase tracking-wider text-slate-700">
            Aksi
          </th>
        </tr>
      </thead>
            ...
            >
              ...
              <td className="px-6 py-5 align-top">
                <div className="flex flex-col gap-1.5">
                  <span className="text-sm font-bold text-slate-800 group-hover:text-primary-600 transition-colors tracking-tight">
                    {ret.docNumber}
                  </span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div
                      ...
                      title={ret.items.map((i) => i.assetName).join(", ")}
                    >
                      {ret.items[0]?.assetName}{" "}
                      {ret.items.length > 1
                        ? `+${ret.items.length - 1} lainnya`
                        : ""}
                    </div>
                  </div>
                </div>
            ...
            <td colSpan={5} className="px-6 py-20 text-center">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Belum ada data pengembalian aset.
                </p>
              </div>
            </td>
          </tr>
