...
const LoanStatusIndicator: React.FC<{ status: LoanRequestStatus }> = ({
  status,
}) => {
  const statusDetails: Record<string, { label: string; className: string }> = {
    [LoanRequestStatus.PENDING]: {
      label: "Menunggu Persetujuan",
      className: "bg-warning-light text-warning-text",
    },
    ...
    [LoanRequestStatus.ON_LOAN]: {
      label: "Dipinjam",
      className: "bg-info-light text-info-text",
    },
    ...
    [LoanRequestStatus.REJECTED]: {
      label: "Ditolak",
      className: "bg-danger-light text-danger-text",
    },
    [LoanRequestStatus.OVERDUE]: {
      label: "Terlambat",
      className: "bg-red-200 text-red-800",
    },
    [LoanRequestStatus.AWAITING_RETURN]: {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full ${details.className}`}
    >
      {details.label}
    </span>
  );
  ...
  switch (loanRequest.status) {
        ...
        actions = (
          <div className="space-y-2">
            <ActionButton
              ...
              disabled={isLoading}
              text="Tinjau & Tetapkan"
              icon={PencilIcon}
              color="success"
            />
            <ActionButton
              onClick={() => onReject(loanRequest)}
              disabled={isLoading}
              text="Tolak Semua"
              icon={CloseIcon}
              color="danger"
    ...
    case LoanRequestStatus.APPROVED:
      if (isAdmin) {
        actions = (
          <ActionButton
            onClick={() => onInitiateHandoverFromLoan(loanRequest)}
            disabled={isLoading}
            text="Buat Dokumen Handover"
            icon={HandoverIcon}
            color="primary"
          />
        );
        ...
        actions = (
          <div className="text-center p-4 bg-gray-50 border border-gray-200 rounded-lg">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Tombol konfirmasi akan muncul setelah peminjam mengajukan
              pengembalian.
            </p>
          </div>
        );
    ...
    case LoanRequestStatus.AWAITING_RETURN:
      if (isAdmin) {
        actions = (
          <ActionButton
            onClick={onOpenReturnConfirmation}
            disabled={isLoading}
        ...
        actions = (
          <div className="text-center p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <p className="text-xs text-blue-600 mt-1">
              Anda telah mengajukan pengembalian. Admin Logistik sedang
              memverifikasi fisik aset.
            </p>
          </div>
        );
      }
      ...
      actions = (
        <div className="text-center p-4 bg-green-50 border border-green-200 rounded-lg">
          ...
          <p className="text-xs text-green-600 mt-1">
            Seluruh aset telah diterima kembali oleh Logistik.
          </p>
        </div>
      );
      break;
    default:
      actions = (
        <div className="text-center p-4 bg-gray-50/70 border border-gray-200/60 rounded-lg">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Tidak ada aksi lebih lanjut untuk permintaan ini.
          </p>
        </div>
      );
  }
    ...
    <div className="p-5 bg-white border border-gray-200/80 rounded-xl shadow-sm">
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2">
            <LoanStatusIndicator status={loanRequest.status} />
          </div>
        </div>
        <button
          onClick={onToggleVisibility}
