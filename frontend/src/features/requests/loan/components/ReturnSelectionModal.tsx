  ...
  const filteredAssets = useMemo(() => {
    ...
    return displayAssets.filter(
      (asset) =>
        ...
        (asset.serialNumber &&
          asset.serialNumber.toLowerCase().includes(lowerQ)),
    );
  ...
  return (
      ...
      footerContent={
        <div className="flex justify-between items-center w-full">
          <div className="text-xs text-gray-500 hidden sm:block">
            <strong>{selected.length}</strong> aset dipilih dari{" "}
            <strong>{displayAssets.length}</strong> tersedia.
          </div>
    ...
    >
      <div className="space-y-4">
        <div
          className={`border p-4 rounded-xl flex items-start gap-3 bg-${themeColor}-50 border-${themeColor}-200`}
        >
          <InfoIcon
            className={`w-5 h-5 mt-0.5 flex-shrink-0 text-${themeColor}-600`}
          />
          <div>
            <p className={`text-sm font-bold text-${themeColor}-800`}>
              {isApprovalMode
                ? "Verifikasi Fisik (Approval)"
                : "Pengajuan Pengembalian (Request)"}
            </p>
            <p
              className={`text-xs mt-1 leading-relaxed text-${themeColor}-700`}
            >
              {isApprovalMode
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-2.5 text-sm bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 outline-none transition-all"
            />
          </div>
          <div
          ...
          >
            <Checkbox
              ...
              onChange={() => {}}
            />
            <label className="text-xs font-bold text-gray-700 cursor-pointer pointer-events-none">
              Pilih Semua
            </label>
          </div>
        </div>
          ...
          {filteredAssets.length > 0 ? (
            filteredAssets.map((asset) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex-shrink-0 w-12 h-12 rounded-lg flex items-center justify-center mr-4 transition-colors
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm font-bold truncate ${isSelected ? "text-gray-900" : "text-gray-800"}`}
                    >
                      {asset.name}
                    </p>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mt-1 text-xs text-gray-500 font-mono">
                      <span className="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200 w-fit">
                        {asset.id}
                      </span>
                      {asset.serialNumber && (
