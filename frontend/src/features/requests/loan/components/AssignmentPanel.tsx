...
export const AssignmentPanel: React.FC<AssignmentPanelProps> = ({
  ...
  const handleQtyChange = (itemId: number, qty: number) => {
    ...
    setItemsState((prev) => {
      ...
      return {
        ...
        [itemId]: {
          ...current,
          approvedQty: validQty,
          assignedAssets: newAssets,
        },
      };
    });
  };
  ...
  const handleStartScan = (itemId: number, index: number) => {
    ...
    setFormScanCallback(() => (data: ParsedScanResult) => {
      ...
      if (data.id)
        assetIdToSet = availableAssets.find((a) => a.id === data.id)?.id;
      else if (data.serialNumber)
        assetIdToSet = availableAssets.find(
          (a) => a.serialNumber === data.serialNumber,
        )?.id;

      if (assetIdToSet) {
        const allAssigned = Object.values(itemsState).flatMap(
          (s: ItemState) => s.assignedAssets,
        );
  ...
  const handleSubmit = () => {
    ...
    for (const item of request.items) {
      ...
      if (!itemState) {
        addNotification(
          `Data internal untuk item ${item.itemName} tidak ditemukan.`,
          "error",
        );
        isValid = false;
        break;
      ...
      const category = assetCategories.find(
        (c) => c.name === matchingAssets[0]?.category,
      ); // Check logic might need robustness if matchingAssets is empty
      // Fallback lookup if matchingAssets empty (e.g. stock 0)
      const typeNameFromItem = availableAssets.find(
        (a) => a.name === item.itemName,
      )?.type;
      const type = category?.types.find(
        (t) => t.name === (matchingAssets[0]?.type || typeNameFromItem),
      );
      ...
      if (isReduced && !itemState.reason.trim()) {
        addNotification(
          `Harap isi alasan revisi/penolakan untuk ${item.itemName}.`,
          "error",
        );
        isValid = false;
        break;
      ...
      if (itemState.approvedQty > 0) {
        if (isBulk) {
          if (matchingAssets.length < itemState.approvedQty) {
            addNotification(
              `Stok ${item.itemName} tidak mencukupi (Tersedia: ${matchingAssets.length}, Butuh: ${itemState.approvedQty}).`,
              "error",
            );
            isValid = false;
            break;
          ...
          const assigned = matchingAssets
            .slice(0, itemState.approvedQty)
            .map((a) => a.id);
          ...
          if (itemState.assignedAssets.some((a) => !a)) {
            addNotification(
              `Harap pilih aset untuk semua slot pada ${item.itemName}.`,
              "error",
            );
            isValid = false;
            break;
          ...
          if (uniqueAssets.size !== itemState.assignedAssets.length) {
            addNotification(
              `Terdeteksi duplikasi aset pada ${item.itemName}.`,
              "error",
            );
            isValid = false;
            break;
          ...
          for (const id of itemState.assignedAssets) {
            if (usedAssetIds.has(id)) {
              addNotification(
                `Aset ${id} sudah digunakan di baris lain.`,
                "error",
              );
              isValid = false;
              break;
    ...
    if (isValid) {
      onConfirm({
        itemStatuses: resultItemStatuses,
        assignedAssetIds: resultAssignedAssets,
      });
    }
  };
    ...
    <div className="bg-white border-2 border-blue-100 rounded-xl shadow-xl animate-fade-in-up relative">
      <div className="bg-blue-50 px-6 py-4 border-b border-blue-100 flex justify-between items-center rounded-t-xl">
        <div>
          <h3 className="text-base sm:text-lg font-bold text-blue-900 flex items-center gap-2">
            <CheckIcon className="w-5 h-5 sm:w-6 sm:h-6 text-blue-700" /> Panel
            Penetapan
          </h3>
      ...
      <div className="p-4 sm:p-6 space-y-6 sm:space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
        {request.items.map((item) => {
          ...
          const matchingAssets = availableAssets.filter(
            (a) => a.name === item.itemName && a.brand === item.brand,
          );
          const allCurrentlyAssigned = Object.values(itemsState)
            .flatMap((s: ItemState) => s.assignedAssets)
            .filter(Boolean);

          const sampleAsset = matchingAssets[0];
          const category = assetCategories.find(
            (c) => c.name === sampleAsset?.category,
          );
          const type = category?.types.find(
            (t) => t.name === sampleAsset?.type,
          );
          const isBulk = type?.trackingMethod === "bulk";
          ...
          return (
            <div
              key={item.id}
              className="border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow bg-white relative"
            >
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-sm text-gray-500 flex-wrap">
                      <span className="px-2 py-0.5 bg-gray-100 rounded text-gray-600 font-medium text-xs">
                        {item.brand}
                      </span>
                      <span>&bull;</span>
                  ...
                  <div className="flex items-center gap-3 bg-gray-50 p-2 sm:p-3 rounded-lg border border-gray-100 w-full md:w-auto justify-between md:justify-start">
                    <div className="flex flex-col items-start md:items-end">
                      <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wide mb-1">
                        Disetujui
                      </label>
                      <div className="flex items-center">
                        <input
                          ...
                          className="w-16 sm:w-20 h-8 sm:h-9 text-center text-sm font-semibold text-gray-900 bg-white border border-gray-300 rounded-md focus:ring-2 focus:ring-primary-600 focus:border-primary-600 outline-none transition-shadow"
                        />
                ...
                <div className="bg-gray-50 p-4 sm:p-5 border-t border-gray-100 relative z-10">
                  <div className="mb-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1">
                    <h5 className="text-sm font-bold text-gray-700 flex items-center gap-2">
                      ...
                      {isBulk
                        ? "Alokasi Stok Material"
                        : `Pilih ${state.approvedQty} Unit Aset`}
                    </h5>
                    <span className="text-xs text-gray-500 bg-white px-2 py-1 rounded border">
                      Stok Tersedia:{" "}
                      <strong>
                        {matchingAssets.length} {unitLabel}
                      </strong>
                    </span>
                  </div>
                  ...
                  {isBulk ? (
                    <div className="p-3 sm:p-4 bg-orange-50 border border-orange-200 rounded-lg flex items-start gap-3">
                      ...
                      <div className="text-sm text-orange-800">
                        ...
                        <p className="mt-1 text-xs sm:text-sm">
                          Item ini adalah material/bulk. Sistem akan secara
                          otomatis mengalokasikan{" "}
                          <strong>
                            {state.approvedQty} {unitLabel}
                          </strong>{" "}
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {Array.from({ length: state.approvedQty }).map(
                        (_, idx) => {
                          const currentVal = state.assignedAssets[idx] || "";
                          const options = matchingAssets
                            .filter(
                              (a) =>
                                !allCurrentlyAssigned.includes(a.id) ||
                                a.id === currentVal,
                            )
                            .map((a) => ({
                              value: a.id,
                              label: `${a.name} (${a.id}) ${a.serialNumber ? `- SN: ${a.serialNumber}` : ""}`,
                            }));
                          ...
                          return (
                            ...
                            >
                              <div className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center bg-white border border-gray-300 rounded-full text-xs font-bold text-gray-500 shadow-sm flex-shrink-0">
                                {idx + 1}
                              </div>
                              <div className="flex-1 min-w-0">
                                <CustomSelect
                                  ...
                                  value={currentVal}
                                  onChange={(val) =>
                                    handleAssetSelect(item.id, idx, val)
                                  }
                                  isSearchable
                              ...
                              <button
                                ...
                                className="p-2 bg-white border border-gray-300 rounded-lg hover:bg-blue-50 text-gray-600 hover:text-primary-600 transition-all shadow-sm flex-shrink-0"
                                title="Scan QR"
                              >
      ...
      <div className="bg-gray-50 px-6 py-5 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3 rounded-b-xl">
        <button
          onClick={onCancel}
          className="w-full sm:w-auto px-6 py-2.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 order-2 sm:order-1"
        >
          Batal
        </button>
        <button
          onClick={handleSubmit}
          className="w-full sm:w-auto px-6 py-2.5 text-sm font-bold text-white bg-primary-600 rounded-lg shadow-sm hover:bg-primary-700 flex items-center justify-center gap-2 order-1 sm:order-2"
        >
