...
const ReturnStatusIndicator: React.FC<{ status: AssetReturnStatus }> = ({
  status,
}) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-bold uppercase tracking-wide rounded-full border ${details.className}`}
    >
      {details.label}
    </span>
  );
...
export const ReturnStatusSidebar: React.FC<ReturnStatusSidebarProps> = ({
    ...
    return (
      <div className="flex flex-col items-center pt-4 space-y-4">
        <button
          ...
          className="flex items-center justify-center w-10 h-10 bg-white border border-gray-300 rounded-full shadow-md text-gray-500 hover:bg-gray-100 hover:text-primary-600 transition-all"
        >
  ...
  const canAct =
    returnDocument.status === AssetReturnStatus.PENDING_APPROVAL && isAdmin;

  // Helper text based on status
  const getStatusMessage = () => {
    if (returnDocument.status === AssetReturnStatus.COMPLETED)
      return "Seluruh item dalam dokumen ini telah diterima dan dimasukkan kembali ke stok.";
    if (returnDocument.status === AssetReturnStatus.APPROVED)
      return "Sebagian item diterima, sebagian ditolak. Cek detail tabel.";
    if (returnDocument.status === AssetReturnStatus.REJECTED)
      return "Seluruh pengembalian ditolak. Aset dikembalikan ke status dipinjam.";
        ...
        {canAct ? (
          <div className="space-y-3 animate-fade-in-up">
            <div className="p-3 bg-blue-50 text-xs text-blue-800 rounded-lg border border-blue-100 leading-relaxed">
              ...
              <p>
                Pastikan kondisi aset yang diterima sesuai dengan laporan
                pengguna sebelum menekan tombol verifikasi.
              </p>
            </div>
            <ActionButton
              onClick={onOpenVerification}
              disabled={isLoading}
              text="Verifikasi & Terima"
              icon={CheckIcon}
              color="success"
            ...
            {returnDocument.verifiedBy && (
              <div className="mt-4 pt-3 border-t border-gray-200">
                ...
                <p className="text-[10px] text-gray-400">
                  {new Date(
                    returnDocument.verificationDate || "",
