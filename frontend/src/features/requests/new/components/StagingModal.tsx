...
export const StagingModal: React.FC<StagingModalProps> = ({
    ...
    return request.items
      ...
      .map((item) => {
        ...
        const remainingQty = Math.max(
          0,
          safeRound(approvedQty - registeredQty),
        );
  ...
  const stockItemsCount = request.items.filter(
    (i) => request.itemStatuses?.[i.id]?.status === "stock_allocated",
  ).length;
      ...
      footerContent={
        <div className="flex justify-end gap-3 w-full">
          ...
          <button
            ...
            className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg shadow-sm hover:bg-primary-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          >
            Lanjut Catat Aset
          </button>
        </div>
    ...
    >
      <div className="space-y-4">
        <div className="p-4 bg-blue-50 border border-blue-100 rounded-lg flex items-start gap-3">
          <ArchiveBoxIcon className="w-6 h-6 text-primary-600 flex-shrink-0 mt-0.5" />
          <div>
            <h4 className="font-bold text-primary-600 text-sm">
              Pilih Barang yang Diterima (Pengadaan Baru)
            </h4>
            <p className="text-sm text-blue-800 mt-1">
              Pilih item dari daftar di bawah untuk dicatat sebagai aset baru.
              Item yang diambil dari stok gudang tidak perlu dicatat lagi di
              sini.
            </p>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center">
                          <input
                            ...
                            className="w-4 h-4 text-primary-600 border-gray-300 focus:ring-primary-600 cursor-pointer disabled:cursor-not-allowed"
                      ...
                      <td className="p-3 font-semibold text-slate-800">
                        {item.itemName}
                        {isCompleted && (
                          <span className="ml-2 text-[9px] text-green-600 bg-green-100 px-1.5 py-0.5 rounded font-bold border border-green-200">
                            SELESAI
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-3 text-center">
                        <div className="flex flex-col items-center">
                          <span
                            className={`font-bold px-2 py-0.5 rounded text-xs ${remainingQty > 0 ? "text-amber-700 bg-amber-100" : "text-slate-400 bg-slate-100"}`}
                          >
                            Sisa: {remainingQty}
                          </span>
                          <span className="text-[10px] text-slate-400 mt-0.5">
                            dari {approvedQty} unit
                          </span>
                        </div>
                      </td>
            ...
            {stockItemsCount > 0 ? (
              <>
                ...
                <h3 className="text-lg font-bold text-slate-800">
                  Semua Item Tersedia dari Stok
                </h3>
                <p className="text-sm text-slate-500 mt-1 max-w-md">
                  {stockItemsCount} item dalam request ini diambil dari stok
                  gudang (Stock Allocated). Anda tidak perlu melakukan
                  pencatatan aset baru. Silakan lanjutkan ke proses Handover.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-sm text-slate-500">
                  Tidak ada item yang perlu dicatat (Pengadaan Baru) dalam
                  request ini.
                </p>
              </>
            )}
        ...
        {stockItemsCount > 0 && stagingItems.length > 0 && (
          <p className="text-xs text-green-700 bg-green-50 p-2 rounded text-center border border-green-100">
            ...
            {stockItemsCount} item lainnya telah dialokasikan dari stok
            (disembunyikan).
          </p>
        )}
      </div>
