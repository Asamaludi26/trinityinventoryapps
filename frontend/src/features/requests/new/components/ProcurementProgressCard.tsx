...
const TimelineStep: React.FC<{
  ...
  const statusClasses = {
    ...
    current: {
      iconBg: "bg-white border-2 border-primary-600 text-primary-600",
      iconText: "text-primary-600",
      text: "text-primary-600 font-extrabold",
    },
    upcoming: {
  ...
  return (
    <div className="flex flex-col items-center text-center w-24 md:w-28 flex-shrink-0 group relative z-10">
      <div
        className={`relative flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm ${currentStatus.iconBg}`}
      >
        {status === "current" && (
          <span className="absolute inline-flex w-full h-full rounded-full opacity-20 animate-ping bg-primary-600"></span>
        )}
        <Icon className={`w-5 h-5 ${currentStatus.iconText}`} />
      </div>
      <p
        className={`mt-3 text-[10px] uppercase tracking-wider ${currentStatus.text} transition-colors duration-300`}
      >
        {title}
      </p>
      {details}
...
export const ProcurementProgressCard: React.FC<{
  request: Request;
  assets: Asset[];
}> = ({ request, assets }) => {
  const registeredAssets = assets.filter(
    (a) => a.poNumber === request.id || a.woRoIntNumber === request.id,
  );
  const lastRegistrationDate =
    registeredAssets.length > 0 && request.isRegistered
      ? new Date(
          Math.max(
  ...
  const getStepStatus = (
    ...
    if (requestIndex === -1) {
      if (
        request.status === ItemStatus.IN_PROGRESS &&
        stepStatus === ItemStatus.PURCHASING
      )
        return "current";
      return "upcoming";
    }
  ...
  const steps = [
    {
      ...
      label: "Disetujui",
      icon: CheckIcon,
      date: request.finalApprovalDate,
    },
    {
      status: ItemStatus.PURCHASING,
      label: "Pengadaan",
      icon: ShoppingCartIcon,
      date: request.purchaseDetails
        ? new Date(
            Math.max(
              ...Object.values(request.purchaseDetails).map(
                (d: PurchaseDetails) => new Date(d.purchaseDate).getTime(),
              ),
            ),
          ).toISOString()
        : null,
    ...
    {
      status: ItemStatus.IN_DELIVERY,
      label: "Pengiriman",
      icon: TruckIcon,
      date: request.actualShipmentDate || null,
    },
    {
    ...
    },
    {
      ...
      label: "Diserahkan",
      icon: HandoverIcon,
      date: request.completionDate || null,
    },
  ];
  ...
  const renderDetails = (
    ...
    return (
      <p className="text-[10px] font-bold text-slate-500 mt-1 whitespace-nowrap bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200">
              ...
              return (
                <React.Fragment key={step.status}>
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`flex-1 h-0.5 mt-5 z-0 relative ${isPrevStepDone ? "bg-primary-600" : "bg-slate-200"} transition-colors duration-700 delay-100`}
                    ></div>
                  )}
                </React.Fragment>
              );
