...
export const RequestForm: React.FC<RequestFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (savedDraft && !prefillItem) {
      ...
      const cat = assetCategories.find((c) =>
        c.types.some((t) =>
          t.standardItems?.some((si) => si.name === prefillItem.name)
        )
      );
      const type = cat?.types.find((t) =>
        t.standardItems?.some((si) => si.name === prefillItem.name)
      );
      const stock = assets.filter(
        (a) =>
          a.name === prefillItem.name && a.status === AssetStatus.IN_STORAGE
      ).length;
      ...
      setItems([
        {
          ...
          quantity: 1,
          keterangan: "Restock kebutuhan operasional.",
          tempCategoryId: cat?.id.toString() || "",
          ...
          if (item.id === id) {
            ...
            if (
              ...
              const cat = assetCategories.find(
                (c) => c.id.toString() === newItem.tempCategoryId
              );
            ...
            if ("itemName" in updates && updates.itemName) {
              ...
              const typeData = cat?.types.find(
                (t) => t.id.toString() === newItem.tempTypeId
              );
              ...
              if (modelData) {
                ...
                newItem.availableStock = assets.filter(
                  (a) =>
                    a.name === updates.itemName &&
                    a.brand === newItem.itemTypeBrand &&
  ...
  const fulfillmentStatus = useMemo(() => {
    ...
    const allInStock = items.every((item) => {
      const actualStock = assets.filter(
        (a) =>
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const cleanItems: RequestItem[] = items.map(
      ({ tempCategoryId, tempTypeId, availableStock, unit, ...rest }) => ({
        ...rest,
      ...
      <div className="flex flex-wrap justify-between items-center bg-white px-5 py-3 rounded-xl border border-slate-200 shadow-sm gap-4 no-print sticky top-2 z-10">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 text-slate-600 border border-slate-200 rounded-lg text-xs font-semibold hover:bg-slate-100 hover:text-tm-primary transition-all disabled:opacity-50"
          >
            {isSavingDraft ? (
          ...
          <form onSubmit={handleFormSubmit} className="space-y-8">
                  ...
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                    {orderType === "Regular Stock" ? (
                      ...
                      <span className="flex justify-between items-center">
                        Tipe Order
                        <span className="text-tm-primary text-[9px] normal-case bg-blue-50 px-1.5 py-0.5 rounded">
                          Wajib Isi Info
                        </span>
                      </span>
                    )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <h3 className="text-sm font-bold text-slate-800 uppercase flex items-center gap-2">
                  <BsBoxSeam className="text-tm-primary w-4 h-4" /> Daftar
                  Barang
                </h3>
                <span
              ...
              <div className="space-y-3">
                {items.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="group relative bg-white border border-slate-200 rounded-lg shadow-sm hover:border-tm-primary/40 hover:shadow-md transition-all p-4"
                    >
                          ...
                          <div className="grid grid-cols-12 gap-3 items-start">
                            <div className="col-span-12 sm:col-span-4">
                              ...
                              <input
                                ...
                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm text-slate-700 focus:bg-white focus:border-tm-primary outline-none transition-colors"
                              />
                            </div>
                            <div className="col-span-8 sm:col-span-6">
                              ...
                              <input
                                ...
                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm text-slate-700 focus:bg-white focus:border-tm-primary outline-none transition-colors placeholder:text-slate-400"
                              />
                            </div>
                            <div className="col-span-4 sm:col-span-2 relative group/qty">
                              ...
                              <div className="relative">
                                <input
                                  ...
                                  min="1"
                                  className="w-full pl-3 pr-8 py-2 bg-white border border-slate-300 rounded-md text-sm font-bold text-center text-slate-800 focus:ring-1 focus:ring-tm-primary focus:border-tm-primary outline-none"
                                />
                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 uppercase group-hover/qty:text-tm-primary">
                                  {item.unit}
                                </span>
                              </div>
                            </div>
                            ...
                            currentType && (
                              <div className="mt-2 p-2 bg-blue-50/50 border border-blue-100 rounded-md flex flex-wrap items-center gap-2 text-xs">
                                ...
                                <button
                                  ...
                                  className={`px-2 py-0.5 rounded border ${
                                    currentType.classification === "material"
                ...
                className={`w-full py-3 border-2 border-dashed rounded-lg font-bold text-xs flex items-center justify-center gap-2 transition-all
                        ${
                          isAtLimit
                            ...
                            : "bg-white border-slate-300 text-slate-500 hover:border-tm-primary hover:text-tm-primary hover:bg-tm-primary/5"
                        }`}
              >
              ...
              >
                ...
                <div>
                  ...
                  <p className="opacity-90 mt-0.5">
                    {fulfillmentStatus === "stock"
                      ? "Semua item tersedia. Langsung ke tahap Serah Terima."
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 no-print">
              ...
              <button
                ...
                className="px-6 py-2 text-sm font-bold text-white bg-slate-900 rounded-lg hover:bg-tm-primary transition-all shadow-md flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
