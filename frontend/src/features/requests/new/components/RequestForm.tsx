...
export const RequestForm: React.FC<RequestFormProps> = ({
  ...
  const [orderType, setOrderType] = useState<OrderType>(
    initialOrderType || "Regular Stock",
  );
  const [justification, setJustification] = useState("");
  const [project, setProject] = useState("");

  // Initialize allocation target with prop or default
  const [allocationTarget, setAllocationTarget] = useState<AllocationTarget>(
    initialAllocationTarget || "Usage",
  );
  ...
  const userDraftKey = useMemo(
    () => `triniti_draft_user_${currentUser.id}`,
    [currentUser.id],
  );
  ...
  const canChooseAllocation = ["Admin Logistik", "Super Admin"].includes(
    currentUser.role,
  );
  ...
  const updateItemState = useCallback(
    (id: number, updates: Partial<RequestItemFormState>) => {
      setItems((prev) =>
        prev.map((item) => {
          if (item.id === id) {
            ...
            if (
              "tempCategoryId" in updates &&
              updates.tempCategoryId !== item.tempCategoryId
            ) {
              nextItem.tempTypeId = "";
              nextItem.itemName = "";
              nextItem.itemTypeBrand = "";
              nextItem.availableStock = 0;
              nextItem.stockDetails = undefined;
            }

            if (
              "tempTypeId" in updates &&
              updates.tempTypeId !== item.tempTypeId
            ) {
              nextItem.itemName = "";
              nextItem.itemTypeBrand = "";
              nextItem.availableStock = 0;
              nextItem.stockDetails = undefined;
              const cat = assetCategories.find(
                (c) => c.id.toString() === nextItem.tempCategoryId,
              );
              const typeData = cat?.types.find(
                (t) => t.id.toString() === updates.tempTypeId,
              );
            ...
            if ("itemName" in updates && updates.itemName) {
              ...
              const typeData = cat?.types.find(
                (t) => t.id.toString() === nextItem.tempTypeId,
              );
              const modelData = typeData?.standardItems?.find(
                (m) => m.name === updates.itemName,
              );
              ...
              if (modelData) {
                ...
                if (!("unit" in updates)) {
                  if (modelData.bulkType === "measurement") {
                    ...
                    nextItem.unit =
                      modelData.unitOfMeasure ||
                      typeData?.unitOfMeasure ||
                      "Unit";
                  }
                }
              }
            ...
            if (nameToCheck && brandToCheck) {
              const stockInfo = checkAvailability(
                nameToCheck,
                brandToCheck,
                Number(qtyToCheck),
                unitToCheck,
  ...
  const handleUnitChange = (itemId: number, newUnit: string) => {
    ...
    const currentCat = assetCategories.find(
      (c) => c.id.toString() === item.tempCategoryId,
    );
    const currentType = currentCat?.types.find(
      (t) => t.id.toString() === item.tempTypeId,
    );
    const selectedModel = currentType?.standardItems?.find(
      (m) => m.name === item.itemName && m.brand === item.itemTypeBrand,
    );
  ...
  const handleCategorySelect = async (itemId: number, value: string) => {
    ...
    const existingCat = assetCategories.find(
  ...
  const handleTypeSelect = async (
    ...
    const existingType = category.types.find(
      (t) => t.name.toLowerCase() === value.trim().toLowerCase(),
    );

    if (existingType) {
      updateItemState(itemId, {
        tempTypeId: existingType.id.toString(),
        unit: existingType.unitOfMeasure || "Unit",
      });
    } else {
      ...
      const updatedCategory = {
        ...category,
        types: [...category.types, newType],
      };
      const updatedCategories = assetCategories.map((c) =>
        c.id === category.id ? updatedCategory : c,
      );
      try {
        await updateCategories(updatedCategories);
        updateItemState(itemId, {
          tempTypeId: newTypeId.toString(),
          unit: "Unit",
        });
      } catch (error) {
  ...
  const handleUpdateTypeConfig = async (
    ...
    categoryId: string,
    typeId: string,
    config: { classification: ItemClassification; unit: string },
  ) => {
    const category = assetCategories.find(
      (c) => c.id.toString() === categoryId,
    );
    if (!category) return;
    const typeIndex = category.types.findIndex(
      (t) => t.id.toString() === typeId,
    );
    if (typeIndex === -1) return;
    const currentType = category.types[typeIndex];
    if (
      currentType.classification === config.classification &&
      currentType.unitOfMeasure === config.unit
    )
      return;
    ...
    const updatedType: AssetType = {
      ...
      trackingMethod:
        config.classification === "material" ? "bulk" : "individual",
  ...
  const handleModelSelect = async (
    ...
    if (category && type) {
      const existingModel = type.standardItems?.find(
      ...
      if (!existingModel) {
        ...
        const updatedType = {
          ...type,
          standardItems: [...(type.standardItems || []), newModel],
        };
        const updatedCategory = {
          ...category,
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (items.some((i) => !i.itemName || i.quantity <= 0)) {
      addNotification(
        "Mohon lengkapi nama item dan kuantitas (minimal 1).",
        "error",
      );
      return;
    }
    ...
    const cleanItems: RequestItem[] = items.map(
      ({
        tempCategoryId,
        tempTypeId,
        availableStock,
    ...
    onCreateRequest({
      items: cleanItems,
      order: { type: orderType, justification, project, allocationTarget },
    });
  };
  ...
  const handleClearDraft = () => {
    if (
      window.confirm("Hapus draf ini? Data yang belum disimpan akan hilang.")
    ) {
      localStorage.removeItem(userDraftKey);
      window.dispatchEvent(new Event("storage"));
      setItems([]);
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-5xl mx-auto">
      <div className="flex flex-wrap justify-between items-center bg-white px-5 py-3 rounded-xl border border-slate-200 shadow-sm gap-4 no-print sticky top-2 z-10">
        <div className="flex items-center gap-3">
          <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2">
            <BsFileEarmarkPdf className="text-primary-600 w-4 h-4" />
            Permintaan Barang
          </h4>
          {lastSaved && (
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 text-slate-600 border border-slate-200 rounded-lg text-xs font-semibold hover:bg-slate-100 hover:text-primary-600 transition-all disabled:opacity-50"
          >
            {isSavingDraft ? (
              <SpinnerIcon className="w-3 h-3" />
            ) : (
              <BsJournalBookmark className="w-3 h-3" />
            )}{" "}
            Simpan
          </button>
          {lastSaved && (
      ...
      <div
        ref={formRef}
        className="bg-white border border-slate-200 rounded-xl shadow-lg relative overflow-hidden"
      >
        ...
        <div className="p-6 sm:p-10">
          ...
          <form onSubmit={handleFormSubmit} className="space-y-8">
              ...
              {canChooseAllocation && (
                <div className="mb-6 pb-4 border-b border-slate-200 relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label
                      className={`relative flex items-start p-4 cursor-pointer rounded-lg border-2 transition-all ${allocationTarget === "Inventory" ? "border-purple-500 bg-purple-50" : "border-gray-200 bg-white hover:bg-gray-50"} ${isAllocationLocked && allocationTarget !== "Inventory" ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
                      <input
                        type="radio"
                      ...
                      <div className="ml-3">
                        <span className="block text-sm font-bold text-gray-900 flex items-center gap-2">
                          <BsArchive className="text-purple-500" /> Stok Gudang
                          (Restock)
                        </span>
                    ...
                    <label
                      className={`relative flex items-start p-4 cursor-pointer rounded-lg border-2 transition-all ${allocationTarget === "Usage" ? "border-blue-500 bg-blue-50" : "border-gray-200 bg-white hover:bg-gray-50"} ${isAllocationLocked && allocationTarget !== "Usage" ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
                      <input
                        type="radio"
                        name="allocationTarget"
                      ...
                      <div className="ml-3">
                        <span className="block text-sm font-bold text-gray-900 flex items-center gap-2">
                          <BsBriefcase className="text-blue-500" /> Penggunaan
                          Sendiri / Proyek
                        </span>
                        <span className="block text-xs text-gray-500 mt-1">
                          Barang untuk keperluan spesifik/pribadi. Wajib
                          handover saat diterima.
                        </span>
                      </div>
                    </label>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-slate-700 text-sm font-medium">
                    <BsPerson className="w-4 h-4 text-slate-400" />{" "}
                    {currentUser.name}
                  </div>
                </div>
                <div>
                ...
                <div className="relative">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">
                    {orderType === "Regular Stock" ? (
                      ...
                      <span className="flex justify-between items-center">
                        Tipe Order
                        <span className="text-primary-600 text-[9px] normal-case bg-blue-50 px-1.5 py-0.5 rounded">
                          Wajib Isi Info
                        </span>
                      </span>
                    )}
                  ...
                  <div className="space-y-2">
                    <select
                      ...
                      className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 focus:ring-2 focus:ring-primary-50 focus:border-primary-600 outline-none transition-all shadow-sm"
                    >
                  ...
                  <div className="animate-fade-in-down w-full">
                    {initialOrderType === "Urgent" && orderType === "Urgent" ? (
                      <div className="w-full p-2.5 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-800 flex items-center gap-3 shadow-sm">
                        ...
                        <div>
                          ...
                          <p className="text-blue-700">
                            Justifikasi telah diisi otomatis oleh sistem
                            berdasarkan peringatan stok.
                          </p>
                        </div>
                      </div>
                    ) : (
                      <input
                        ...
                        className="w-full px-3 py-2 bg-white border-2 border-yellow-100 focus:border-primary-400 rounded-lg text-sm text-slate-700 placeholder:text-slate-400 focus:ring-0 outline-none transition-all"
                      />
                    )}
                  </div>
                )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <h3 className="text-sm font-bold text-slate-800 uppercase flex items-center gap-2">
                  <BsBoxSeam className="text-primary-600 w-4 h-4" /> Daftar
                  Barang
                </h3>
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${isAtLimit ? "bg-red-50 text-red-600 border-red-100" : "bg-slate-100 text-slate-500 border-slate-200"}`}
                >
              ...
              <div className="space-y-3">
                {items.map((item, idx) => {
                  ...
                  const isNewType =
                    currentType && Date.now() - currentType.id < 60000;
                  ...
                  const selectedModel = availableModels.find(
                    (m) =>
                      ...
                      m.brand === item.itemTypeBrand,
                  );
                  const isMeasurement =
                    selectedModel?.bulkType === "measurement";
                  const isContainerMode =
                    item.unit === selectedModel?.unitOfMeasure;
                  ...
                  let availableUnitOptions = STANDARD_UNIT_OPTIONS.map((u) => ({
                    value: u,
                    label: u,
                  }));
                  if (isMeasurement && selectedModel) {
                    availableUnitOptions = [
                      {
                        value: selectedModel.unitOfMeasure || "Hasbal",
                        label: `${selectedModel.unitOfMeasure || "Hasbal"} (Fisik)`,
                      },
                      {
                        value: selectedModel.baseUnitOfMeasure || "Meter",
                        label: `${selectedModel.baseUnitOfMeasure || "Meter"} (Eceran)`,
                      },
                    ];
                  } else if (currentType?.unitOfMeasure) {
                    availableUnitOptions = [
                      {
                        value: currentType.unitOfMeasure,
                        label: currentType.unitOfMeasure,
                      },
                    ];
                  }
                  ...
                  return (
                    <div
                      key={item.id}
                      className="group relative bg-white border border-slate-200 rounded-lg shadow-sm hover:border-primary-600/40 hover:shadow-md transition-all p-4"
                    >
                      {items.length > 1 && (
                        <button
                          type="button"
                      ...
                      <div className="flex gap-4">
                        <div className="flex-shrink-0 w-6 h-6 flex items-center justify-center bg-slate-100 text-slate-500 text-xs font-bold rounded mt-1 group-hover:bg-primary-600 group-hover:text-white transition-colors">
                          {idx + 1}
                        </div>
                        <div className="flex-1 space-y-3">
                          <div className="grid grid-cols-12 gap-3">
                            <div className="col-span-12 sm:col-span-4">
                              ...
                              <CreatableSelect
                                ...
                                onChange={(val) =>
                                  handleCategorySelect(item.id, val)
                                }
                          ...
                          <div className="grid grid-cols-12 gap-3 items-start">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  updateItemState(item.id, {
                                    itemTypeBrand: e.target.value,
                                  })
                                }
                                placeholder="Cth: Mikrotik"
                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm text-slate-700 focus:bg-white focus:border-primary-600 outline-none transition-colors"
                              />
                            </div>
                            <div className="col-span-4 sm:col-span-2">
                              ...
                              <div className="relative group/stock">
                                <input
                                  ...
                                  className={`w-full px-3 py-2 border rounded-md text-sm font-bold text-center cursor-default focus:outline-none ${hasStock ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-slate-100 border-slate-200 text-slate-400"} ${isFragmented ? "border-amber-400 bg-amber-50 text-amber-800" : ""}`}
                                />
                                {isFragmented && (
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 p-2 bg-slate-800 text-white text-[10px] rounded shadow-lg opacity-0 group-hover/stock:opacity-100 transition-opacity pointer-events-none z-20">
                                  ...
                                  <div className="flex justify-between">
                                    <span>Unit Fisik (Gudang):</span>{" "}
                                    <span className="font-mono">
                                      {physicalStock}{" "}
                                      {selectedModel?.unitOfMeasure || "Unit"}
                                    </span>
                                  </div>
                                  <div className="flex justify-between text-amber-300">
                                    <span>Ter-reservasi:</span>{" "}
                                    <span className="font-mono">
                                      {reservedStock} Unit
                                    </span>
                                  </div>
                                  ...
                                  {isFragmented && (
                                    <div className="mt-1 text-amber-400 italic">
                                      ⚠️ Stok terpecah/fragmented.
                                    </div>
                                  )}
                                </div>
                              </div>
                            ...
                            <div className="col-span-8 sm:col-span-5">
                              ...
                              <input
                                ...
                                placeholder="Spesifikasi / Keperluan..."
                                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-sm text-slate-700 focus:bg-white focus:border-primary-600 outline-none transition-colors placeholder:text-slate-400"
                              />
                            </div>
                            <div className="col-span-12 sm:col-span-2 relative">
                              ...
                              <div className="flex rounded-md shadow-sm">
                                <input
                                  ...
                                  className="w-1/2 pl-3 pr-1 py-2 bg-white border border-slate-300 rounded-l-md text-sm font-bold text-center text-slate-800 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 outline-none z-10"
                                />
                                <select
                                  value={item.unit}
                                  onChange={(e) =>
                                  ...
                                  className="w-1/2 pl-2 pr-6 py-2 bg-slate-50 border-y border-r border-slate-300 rounded-r-md text-[10px] font-semibold text-slate-700 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 outline-none appearance-none cursor-pointer hover:bg-slate-100"
                          ...
                          {isMeasurement && selectedModel && (
                            <div className="mt-2 p-2 bg-indigo-50 border border-indigo-100 rounded-md flex flex-col sm:flex-row items-start sm:items-center gap-2 text-xs text-indigo-800 animate-fade-in-up">
                              ...
                              <span>
                                1 {selectedModel.unitOfMeasure} (Fisik) ={" "}
                                <strong>
                                  {selectedModel.quantityPerUnit}{" "}
                                  {selectedModel.baseUnitOfMeasure}
                                </strong>
                                .
                              </span>
                              {isContainerMode ? (
                                <span className="ml-auto text-[10px] bg-indigo-200 px-2 py-0.5 rounded text-indigo-900 font-bold">
                                  Anda meminta {item.quantity} Unit Fisik (Total{" "}
                                  {item.quantity *
                                    (selectedModel.quantityPerUnit || 0)}{" "}
                                  {selectedModel.baseUnitOfMeasure})
                                </span>
                              ) : (
                          ...
                          {!isMeasurement &&
                            (isNewType ||
                              (currentType && !currentType.classification)) &&
                            currentCat &&
                            currentType && (
                              <div className="mt-2 p-2 bg-blue-50/50 border border-blue-100 rounded-md flex flex-wrap items-center gap-2 text-xs">
                                <span className="font-bold text-blue-700 flex items-center gap-1">
                                  <BsLightningChargeFill /> Konfigurasi:
                                </span>
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleUpdateTypeConfig(
                                      ...
                                      item.tempCategoryId,
                                      item.tempTypeId,
                                      { classification: "asset", unit: "Unit" },
                                    )
                                  }
                                  className={`px-2 py-0.5 rounded border ${currentType.classification === "asset" ? "bg-blue-100 border-blue-300 text-blue-800" : "bg-white border-slate-200 text-slate-500"}`}
                                >
                                  Device
                                </button>
                                <button
                                  ...
                                  className={`px-2 py-0.5 rounded border ${currentType.classification === "material" ? "bg-orange-100 border-orange-300 text-orange-800" : "bg-white border-slate-200 text-slate-500"}`}
                                >
                                  Material
                                </button>
                                <select
                                  ...
                                  onChange={(e) =>
                                    handleUpdateTypeConfig(
                                      ...
                                      {
                                        classification:
                                          currentType.classification || "asset",
                                        unit: e.target.value,
                                      },
                                    )
                                  }
                                  className="px-1 py-0.5 border border-slate-200 rounded text-slate-700 bg-white outline-none"
                                >
              ...
              <button
                ...
                disabled={isAtLimit}
                className={`w-full py-3 border-2 border-dashed rounded-lg font-bold text-xs flex items-center justify-center gap-2 transition-all ${isAtLimit ? "bg-slate-50 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-slate-300 text-slate-500 hover:border-primary-600 hover:text-primary-600 hover:bg-primary-50"}`}
              >
            ...
            {fulfillmentStatus !== "invalid" && (
              <div
                className={`p-3 rounded-lg border flex items-start gap-3 text-xs ${fulfillmentStatus === "stock" ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-blue-50 border-blue-200 text-blue-800"}`}
              >
                <div
                  className={`p-1.5 rounded-full ${fulfillmentStatus === "stock" ? "bg-emerald-100" : "bg-blue-100"}`}
                >
                  {fulfillmentStatus === "stock" ? (
                    <BsCheckCircleFill className="w-3.5 h-3.5" />
                  ) : (
                    <BsCart className="w-3.5 h-3.5" />
                  )}
                </div>
                <div>
                  ...
                  <p className="opacity-90 mt-0.5">
                    {fulfillmentStatus === "stock"
                      ...
                      : "Sebagian item tidak tersedia. Masuk alur persetujuan pengadaan."}
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 no-print">
              ...
              <button
                type="submit"
                disabled={isLoading || fulfillmentStatus === "invalid"}
                className="px-6 py-2 text-sm font-bold text-white bg-slate-900 rounded-lg hover:bg-primary-600 transition-all shadow-md flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading && <SpinnerIcon className="w-4 h-4" />} Kirim
                Permintaan
              </button>
            </div>
          </form>
