...
export const RequestStatusIndicator: React.FC<{ status: ItemStatus }> = ({
  status,
}) => {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full ${statusDetails.className}`}
    >
      <statusDetails.Icon
        className={`flex-shrink-0 w-3.5 h-3.5 ${status === ItemStatus.IN_PROGRESS ? "animate-spin" : ""}`}
      />
...
export const OrderIndicator: React.FC<{ order: OrderDetails }> = ({
  order,
}) => {
