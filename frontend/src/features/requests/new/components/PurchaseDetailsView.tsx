...
const canViewPrice = (user: User) =>
  hasPermission(user, "requests:approve:purchase");

export const PurchaseDetailsView: React.FC<{
  request: Request;
  details: Record<number, PurchaseDetails>;
  currentUser: User;
  totalValue: number;
}> = ({ request, details, currentUser, totalValue }) => (
  <section>
                ...
                <tr key={item.id} className="bg-red-50/30 text-slate-400">
                  <td className="p-3 font-semibold">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-1.5 py-0.5 text-[9px] font-bold text-white bg-red-500 rounded uppercase">
                        Ditolak
                      </span>
                    </div>
                  </td>
                  <td
                    colSpan={canViewPrice(currentUser) ? 8 : 6}
                    className="p-3 text-xs italic"
                  >
                    {request.itemStatuses?.[item.id]?.reason ||
                      "Item ditolak saat proses review."}
                  </td>
                </tr>
              );
            }
            ...
            if (itemDetails) {
              ...
              return (
                ...
                >
                  ...
                  {canViewPrice(currentUser) && (
                    <>
                      <td className="p-3 text-right font-mono font-normal text-slate-700">
                  ...
                  <td className="p-3 text-slate-600 text-xs">
                    <div className="font-mono font-normal bg-slate-100 px-1.5 py-0.5 rounded w-fit mb-1 border border-slate-200 text-slate-700">
                  ...
                  <td className="p-3 text-slate-600 text-xs">
                    <div className="font-normal text-slate-700">
                      {itemDetails.filledBy}
                    </div>
                    <div className="text-slate-400">
                      {new Date(itemDetails.fillDate).toLocaleDateString(
                        "id-ID",
                      )}
                    </div>
                  </td>
        ...
        {canViewPrice(currentUser) && (
          <tfoot className="bg-slate-50 border-t border-slate-200">
            <tr>
              <td
                colSpan={8}
                className="p-4 text-center font-bold text-slate-800 text-base"
              >
                Estimasi Total:{" "}
                <span className="text-xl text-primary-600 ml-2 font-mono">
