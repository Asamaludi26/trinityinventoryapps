    ...
    <div className="space-y-4 relative">
      {activities.map((activity) => {
        const replies = allActivities
          .filter((reply) => reply.parentId === activity.id)
          .sort(
            (a, b) =>
        ...
        if (activity.type === "status_change") {
          return (
            <div key={activity.id} className="relative text-center my-8">
              ...
              <span className="bg-white px-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest border border-slate-100 rounded-full py-0.5">
        ...
        if (activity.type === "revision") {
          return (
            <div
              key={activity.id}
              className={`flex items-start space-x-3 ${level > 0 ? "ml-10" : ""}`}
            >
              <div className="relative">
                {/* Vertical Line for Thread */}
                {replies.length > 0 && (
                  <div className="absolute top-8 left-1/2 w-0.5 h-full bg-slate-200 -z-10"></div>
                )}
                <Avatar
                  name={activity.author}
                  className="w-8 h-8 flex-shrink-0 text-xs shadow-sm ring-2 ring-white"
                />
              </div>
                  ...
                  <div className="space-y-3 relative z-10">
                    {activity.payload.revisions?.map((rev, index) => {
                      const rejectedQuantity =
                        rev.originalQuantity - rev.approvedQuantity;
                      const isFullyRejected = rev.approvedQuantity === 0;

                      return (
                        <div
                          key={index}
                          className="text-sm border-t border-amber-200/50 pt-2 first:border-t-0 first:pt-0"
                        >
                          ...
                          {isFullyRejected ? (
                            <div className="flex items-center gap-2 text-xs">
                              <span className="font-bold text-red-600 bg-red-50 px-1.5 py-0.5 rounded uppercase text-[10px] border border-red-100">
                                Ditolak
                              </span>
                              <span className="text-slate-400 line-through font-medium">
                                {rev.originalQuantity} diajukan
                              </span>
                            </div>
                          ) : (
                            <div className="flex flex-wrap items-center gap-2 text-xs">
                              <span className="font-bold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded uppercase text-[10px] border border-amber-200">
                                Revisi
                              </span>
                              <span className="text-slate-400 line-through font-medium">
                                {rev.originalQuantity} diajukan
                              ...
                              <span className="font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100">
                                {rev.approvedQuantity} disetujui
                              </span>
                            </div>
                          )}

                          <p className="text-xs text-amber-900/80 italic mt-2 bg-white/50 p-2 rounded border border-amber-100/50">
                            "{rev.reason}"
                          </p>
                        </div>
                      );
        ...
        return (
          <div key={activity.id} className="relative group">
            ...
            >
              ...
              <div className="flex-1">
                {isEditingThis ? (
                  <div className="bg-white border border-primary-600 ring-2 ring-primary-50 rounded-xl p-3 shadow-md z-20 relative">
                    <textarea
                      ref={editInputRef}
                      value={editText}
                      onChange={(e) => {
                    ...
                    <div className="flex items-center gap-2 mt-3 pt-2 border-t border-slate-100">
                      <button
                        onClick={onSaveEdit}
                        className="px-3 py-1 text-xs font-bold text-white bg-primary-600 rounded-md shadow-sm hover:bg-primary-700"
                      >
                        Simpan
                      </button>
                      <button
                        onClick={onCancelEdit}
                        className="px-3 py-1 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-md"
                      >
                        Batal
                      </button>
                  ...
                  <div className="relative p-3.5 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md hover:border-slate-300 transition-all z-10">
                    ...
                    <div className="flex items-center justify-between mb-1 relative z-10">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => onStartReply(activity)}
                          className="p-1 text-slate-400 hover:text-primary-600 hover:bg-blue-50 rounded"
                          title="Balas"
                        >
