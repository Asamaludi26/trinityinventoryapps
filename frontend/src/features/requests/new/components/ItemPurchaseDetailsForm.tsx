...
export const ItemPurchaseDetailsForm: React.FC<
  ItemPurchaseDetailsFormProps
> = ({ item, approvedQuantity, onChange, isDisabled = false, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setPurchasePrice((prev) =>
        initialData.purchasePrice !== prev
          ? (initialData.purchasePrice ?? "")
          : prev,
      );
      setVendor((prev) =>
        initialData.vendor !== prev ? (initialData.vendor ?? "") : prev,
      );
      setPoNumber((prev) =>
        initialData.poNumber !== prev ? (initialData.poNumber ?? "") : prev,
      );
      setInvoiceNumber((prev) =>
        initialData.invoiceNumber !== prev
      ...
      if (
        ...
        (!purchaseDate ||
          new Date(initialData.purchaseDate).getTime() !==
      ...
      if (
        ...
        (!warrantyEndDate ||
          new Date(initialData.warrantyEndDate).getTime() !==
  ...
  const triggerChange = (
    overrides: Partial<Omit<PurchaseDetails, "filledBy" | "fillDate">>,
  ) => {
    const currentData = {
  ...
  const handlePurchaseDateChange = (date: Date | null) => {
    ...
    triggerChange({
      ...
      warrantyEndDate: newWarrantyEnd
        ? newWarrantyEnd.toISOString().split("T")[0]
        : null,
    });
  };
  ...
  const handleWarrantyEndDateChange = (date: Date | null) => {
    ...
    if (purchaseDate && date && date > purchaseDate) {
      ...
      let months =
        (date.getFullYear() - pDate.getFullYear()) * 12 +
        (Number(date.getMonth()) - Number(pDate.getMonth()));
    ...
    triggerChange({
      warrantyEndDate: date ? date.toISOString().split("T")[0] : null,
    });
  };

  // Calculate subtotal for display
  const calculatedSubtotal =
    (typeof purchasePrice === "number" ? purchasePrice : 0) * approvedQuantity;

  return (
    <div
      className={`border border-slate-200 rounded-lg shadow-sm transition-all duration-300 ${isDisabled ? "bg-slate-50 opacity-60" : "bg-white hover:border-primary-400"}`}
    >
      <button
        type="button"
      ...
      >
        <div className="flex items-center gap-4">
          <div
            className={`flex items-center justify-center w-10 h-10 rounded-lg text-sm font-bold shadow-sm ${isDisabled ? "bg-slate-200 text-slate-500" : "bg-primary-600 text-white"}`}
          >
            {approvedQuantity}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span
                className={`text-sm font-bold ${isDisabled ? "line-through text-slate-400" : "text-slate-900"}`}
              >
                {item.itemName}
              </span>
              {isDisabled && (
                <span className="px-2 py-0.5 text-[10px] font-bold uppercase text-white bg-red-500 rounded-full tracking-wider">
                  Ditolak
                </span>
              )}
            </div>
            <p className="text-xs text-slate-500 font-medium mt-0.5 bg-slate-100 px-2 py-0.5 rounded w-fit">
              {item.itemTypeBrand}
            </p>
          </div>
        </div>
      ...
      <div
        className={`transition-all duration-300 ease-in-out overflow-hidden ${isExpanded && !isDisabled ? "max-h-[1000px]" : "max-h-0"}`}
      >
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className="block w-full py-2.5 pl-10 pr-3 text-slate-900 bg-white border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-primary-50 focus:border-primary-600 transition-all font-normal"
                  />
                </div>
                {/* Subtotal Helper */}
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="block w-full px-3 py-2.5 text-slate-900 bg-white border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-primary-50 focus:border-primary-600 transition-all font-normal"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1.5">
                  No. Purchase Order <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={poNumber}
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-3 bg-slate-50 p-4 rounded-lg border border-slate-100">
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  className="block w-full px-3 py-2 text-slate-900 bg-white border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-primary-50 focus:border-primary-600 transition-all font-normal"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wide mb-1.5">
                ...
                <DatePicker
                  id={`we-${item.id}`}
                  selectedDate={warrantyEndDate}
                  onDateChange={handleWarrantyEndDateChange}
                />
              </div>
            </div>
          </div>
