      ...
      footerContent={
        <div className="flex items-center justify-between w-full">
          <p className="text-xs text-slate-600 font-bold hidden sm:block">
            <span className="text-red-600">*</span> Perubahan wajib menyertakan
            alasan jelas.
          </p>
            ...
            <button
              onClick={() => {
                ...
                Object.keys(adjustments).forEach((id) => {
                  ...
                  final[numId] = {
                    approvedQuantity: Math.floor(
                      Number(adjustments[numId].approvedQuantity),
                    ),
              ...
              className="flex-1 sm:flex-none inline-flex items-center justify-center px-8 py-2.5 text-sm font-bold text-white bg-primary-600 rounded-xl shadow-lg shadow-primary-600/20 hover:bg-primary-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none transition-all"
            >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 bg-slate-100 rounded-2xl border border-slate-200">
          ...
          <div className="flex items-center gap-3 px-4 py-2 bg-white rounded-xl shadow-sm border border-slate-200">
            ...
            <p className="text-xs font-bold text-slate-700 leading-tight">
              Review kuantitas item untuk menyesuaikan <br /> dengan stok atau
              kebijakan anggaran.
            </p>
          </div>
        </div>
          ...
          {request.items.map((item) => {
            ...
            if (isPreviouslyRejected) {
              return (
                <div
                  key={item.id}
                  className="p-4 border-2 border-slate-100 bg-slate-50 rounded-2xl opacity-70"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <h4 className="font-black text-slate-500 text-base line-through">
                        {item.itemName}
                      </h4>
                      <span className="px-2 py-0.5 text-[9px] font-black uppercase bg-red-100 text-red-600 rounded border border-red-200">
                  ...
                  <p className="text-xs text-slate-400 mt-2 italic">
                    Item ini telah ditolak pada tahap approval sebelumnya dan
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="px-2 py-0.5 text-[9px] font-black uppercase bg-slate-200 text-slate-700 rounded border border-slate-300">
                        {item.itemTypeBrand}
                      </span>
                    </div>
                    ...
                    <div className="text-sm text-slate-600 font-bold flex flex-col gap-1">
                      {isReducedPreviously ? (
                        <div className="flex items-center gap-2 text-amber-700">
                          <span>
                            Permintaan Tahap Ini:{" "}
                            <span className="text-slate-900 font-black text-lg">
                              {maxQty} {item.unit || "Unit"}
                            </span>
                          </span>
                          <span className="text-[10px] bg-amber-100 px-1.5 py-0.5 rounded border border-amber-200">
                            (Direvisi dari {originalUserQty})
                          </span>
                        </div>
                      ) : (
                        <span>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-24 text-center text-2xl font-black text-primary-600 border-b-2 border-primary-600/20 focus:border-primary-600 bg-transparent outline-none transition-all"
                          placeholder="0"
                        />
                    ...
                    <div className="flex justify-center items-center gap-2 mt-2">
                      ...
                      {isReducedPreviously && (
                        <span className="text-[10px] text-amber-600 font-bold">
                          (Sudah dikurangi dari {originalUserQty})
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {isModified && (
                  <div className="mt-6 animate-fade-in-down">
                    <div className="flex items-center gap-2 mb-2">
                      <label
                        className={`text-[10px] font-black uppercase tracking-widest ${action === "reject" ? "text-red-700" : "text-amber-800"}`}
                      >
                      ...
                      <textarea
                        ...
                        placeholder={
                          action === "reject"
                            ...
                            : "Berikan alasan perubahan kuantitas..."
                        }
