...
interface RequestDetailPageProps {
  ...
  onSubmitForCeoApproval: (
    id: string,
    data: Record<number, Omit<PurchaseDetails, "filledBy" | "fillDate">>,
  ) => void;
...
const NewRequestDetailPage: React.FC<RequestDetailPageProps> = (props) => {
  ...
  const [itemPurchaseDetails, setItemPurchaseDetails] = useState<
    Record<number, Omit<PurchaseDetails, "filledBy" | "fillDate">>
  >({});
  ...
  useEffect(() => {
    if (request.purchaseDetails) {
      const mappedDetails: Record<
        number,
        Omit<PurchaseDetails, "filledBy" | "fillDate">
      > = {};
      let hasData = false;
      ...
      if (hasData) {
        ...
        setItemPurchaseDetails((prev) => {
          const isDifferent =
            JSON.stringify(prev) !== JSON.stringify(mappedDetails);
  ...
  const isStagingComplete = useMemo(() => {
    ...
    const itemsToProcess = request.items.filter((item) => {
      ...
      if (status?.status === "stock_allocated" || status?.status === "rejected")
        return false;
  ...
  const handleStagingAction = async () => {
    if (isStagingComplete) {
      ...
      const updateActivity: Activity = {
        ...
        type: "status_change",
        payload: {
          text: "Pencatatan aset selesai. Status diubah menjadi Siap Serah Terima.",
        },
      };
  ...
  const handleSaveEdit = async () => {
    ...
    const updatedLog = (request.activityLog || []).map((act) =>
      act.id === editingActivityId
        ? { ...act, payload: { ...act.payload, text: editText.trim() } }
        : act,
    );
  ...
  const handleDelete = async () => {
    ...
    const updatedLog = (request.activityLog || []).filter(
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      ...
      addNotification(
        `Permintaan #${request.id} berhasil dibatalkan.`,
        "success",
      );
  ...
  const isPurchaseFormValid = useMemo(() => {
    if (request.status === ItemStatus.LOGISTIC_APPROVED) {
      ...
      return itemsRequiringPurchase.every((item) => {
        ...
        const hasInvoice =
          detail.invoiceNumber && detail.invoiceNumber.trim().length > 0;
  ...
  const handlePurchaseDetailChange = useCallback(
    (
      itemId: number,
      details: Omit<PurchaseDetails, "filledBy" | "fillDate">,
    ) => {
      setItemPurchaseDetails((prev) => ({
        ...prev,
        [itemId]: details,
  ...
  const handleFinalSubmitForApproval = () => {
    if (!isPurchaseFormValid) {
      addNotification(
        "Harap isi semua detail pembelian yang wajib diisi untuk item yang disetujui (selain stok/ditolak).",
        "error",
      );
      return;
    }
  ...
  return (
    <DetailPageLayout
      ...
      headerActions={
        <div className="flex items-center gap-2 no-print">
          <button
            onClick={handlePrint}
            className="hidden sm:inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-bold text-slate-700 bg-white border border-slate-300 rounded-xl shadow-sm hover:bg-slate-50 transition-all"
          >
      ...
      mainColClassName={
        isActionSidebarExpanded ? "lg:col-span-8" : "lg:col-span-11"
      }
      asideColClassName={
        isActionSidebarExpanded ? "lg:col-span-4" : "lg:col-span-1"
      }
      aside={
        ...
        <div className="sticky top-20">
          <RequestStatusSidebar
            ...
            onToggleVisibility={() =>
              setIsActionSidebarExpanded((prev) => !prev)
            }
    ...
    >
      <div className="space-y-6">
        <div
          ref={printRef}
          className="p-8 bg-white border border-slate-200 shadow-sm rounded-2xl space-y-8 relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-primary-600 to-primary-500"></div>
          <Letterhead />
          ...
          {request.isPrioritizedByCEO && (
            <div className="p-4 flex items-start gap-3 text-sm bg-purple-50 border border-purple-200 rounded-xl text-purple-800 shadow-sm animate-fade-in-down">
              ...
              <div>
                ...
                <p>
                  Permintaan ini telah diprioritaskan oleh CEO pada{" "}
                  <strong>
                    {new Date(request.ceoDispositionDate!).toLocaleString(
                      "id-ID",
                    )}
                  </strong>
                  .
            ...
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 shadow-sm animate-fade-in-down no-print">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    Seluruh item telah selesai dicatat/dialokasikan. Silakan
                    klik tombol <strong>"Selesai & Siap Handover"</strong> di
                    sidebar kanan untuk mengubah status menjadi{" "}
                    <strong>Siap Serah Terima</strong>.
                  </p>
                </div>
              </div>
            </div>
          ...
          {request.progressUpdateRequest &&
            !request.progressUpdateRequest.isAcknowledged &&
            currentUser.role === "Admin Purchase" && (
              <div className="p-4 flex items-start gap-4 text-sm bg-blue-50 border border-blue-200 rounded-xl text-blue-800 no-print shadow-sm animate-fade-in-down">
                ...
                <div>
                  <p className="font-bold uppercase tracking-wide text-xs mb-1">
                    Permintaan Update Progres
                  </p>
                  <p className="mt-1 mb-3">
                    <span className="font-semibold">
                      {request.progressUpdateRequest.requestedBy}
                    </span>{" "}
                    meminta update progres untuk permintaan ini.
                  </p>
                  <button
                    onClick={props.onAcknowledgeProgressUpdate}
                    disabled={props.isLoading}
                    className="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-bold text-white bg-primary-600 rounded-lg shadow-sm hover:bg-primary-700 transition-all"
                  >
                    {props.isLoading ? <SpinnerIcon /> : <CheckIcon />} Tandai
                    Sudah Dilihat
                  </button>
                </div>
              </div>
          ...
          <div className="text-center pb-4 border-b border-slate-100">
            <h3 className="text-2xl font-black uppercase text-slate-800 tracking-tight font-sans">
          ...
          <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-8">
              <PreviewItem
                label="No. Request"
                value={
                  <div className="font-mono font-bold text-slate-700 flex items-center gap-2 text-base">
                    <BsHash className="text-slate-400" /> {request.id}
                  </div>
                }
              />
              <PreviewItem
                ...
                value={
                  <div className="font-mono text-slate-700 flex items-center gap-2">
                    <BsFileEarmarkText className="text-slate-400" />{" "}
                    {request.docNumber || "-"}
                  </div>
                }
              />
              <PreviewItem
                ...
                value={
                  <div className="flex items-center gap-2">
                    <BsCalendarEvent className="text-slate-400" />{" "}
              ...
              <PreviewItem
                ...
                value={
                  <div className="flex items-center gap-2">
                    <BsPerson className="text-slate-400" /> {request.requester}
                  </div>
                }
              />
              <PreviewItem
                ...
                value={
                  <div className="flex items-center gap-2">
                    <BsBuilding className="text-slate-400" /> {request.division}
                  </div>
                }
              />
                ...
                <PreviewItem
                  ...
                  value={
                    <span className="font-bold text-slate-800">
                      {request.order.project}
                    </span>
                  }
                />
              )}
              {request.order.type === "Urgent" && (
                <PreviewItem label="Justifikasi" fullWidth>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {request.items.map((item, index) => {
                    ...
                    const isAdjusted =
                      hasApprovalRecord && approvedQuantity !== item.quantity;

                    const isRejected =
                      hasApprovalRecord && approvedQuantity === 0;
                    const isPartiallyApproved =
                      isAdjusted &&
                      approvedQuantity! > 0 &&
                      approvedQuantity! < item.quantity;
                    const isStockAllocated =
                      itemStatus?.status === "stock_allocated";
                    ...
                    let category = assetCategories.find(
                      (c) => c.id.toString() === item.categoryId?.toString(),
                    );
                    let type = category?.types.find(
                      (t) => t.id.toString() === item.typeId?.toString(),
                    );
                    ...
                    const isItemFullyRegistered =
                      !isRejected &&
                      !isStockAllocated &&
                      registeredCount >= targetQty;
                      ...
                      <tr key={item.id} className={rowClass}>
                            ...
                            <div className="mt-1">
                              {isMeasurement ? (
                                ...
                                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] bg-orange-50 text-orange-700 border border-orange-100 font-bold uppercase tracking-wider">
                                  <BsLightningFill className="w-2.5 h-2.5" />{" "}
                                  Material
                                </span>
                              ) : (
                        ...
                        <td className="p-4 font-semibold">
                          <div className="flex flex-col gap-1">
                            ...
                            <span
                              className={`text-xs ${isRejected ? "text-slate-400" : "text-slate-500 font-medium"}`}
                            >
                              {item.itemTypeBrand}
                            </span>
                            ...
                            <div className="flex gap-2 flex-wrap mt-1">
                              {isPartiallyApproved && (
                                <span className="px-2 py-0.5 text-[9px] font-bold text-white bg-amber-500 rounded uppercase tracking-wider shadow-sm">
                                  Revisi
                                </span>
                              )}
                              {isRejected && (
                                <span className="px-2 py-0.5 text-[9px] font-bold text-white bg-red-500 rounded uppercase tracking-wider shadow-sm">
                                  Ditolak
                                </span>
                              )}
                              {isStockAllocated && (
                                <span className="px-2 py-0.5 text-[9px] font-bold text-emerald-800 bg-emerald-100 border border-emerald-200 rounded uppercase tracking-wider shadow-sm">
                                  Stok
                                </span>
                              )}
                              {isItemFullyRegistered && (
                                <span className="px-2 py-0.5 text-[9px] font-bold text-blue-800 bg-blue-100 border border-blue-200 rounded uppercase tracking-wider shadow-sm flex items-center gap-1">
                          ...
                          <div className="flex flex-col items-center">
                            {isAdjusted ? (
                              <>
                                ...
                                <strong
                                  className={`text-lg font-bold ${isRejected ? "text-red-500" : "text-amber-600"}`}
                                >
                                  {approvedQuantity}
                                </strong>
                              </>
                            ...
                            <span className="text-[9px] uppercase font-bold text-slate-500 mt-0.5 tracking-wide bg-slate-100 px-1.5 rounded">
                              {displayUnit}
                            </span>
                          </div>
                        </td>
                        <td
                          className={`p-4 text-sm ${isRejected ? "text-slate-400" : "text-slate-600"}`}
                        >
                          ...
                          {itemStatus?.reason && (
                            <div className="not-italic mt-2 font-semibold text-amber-800 bg-amber-50 px-3 py-2 rounded-lg border border-amber-100 text-xs">
                              <span className="font-bold uppercase tracking-wide text-amber-600 mb-1 block">
                                Catatan Admin:
                              </span>
                              {itemStatus.reason}
                            </div>
          ...
          {request.status === ItemStatus.LOGISTIC_APPROVED &&
            canViewPrice &&
            hasItemsToProcess && (
              <section className="p-6 mt-8 border-2 border-dashed border-primary-600/30 bg-blue-50/30 rounded-2xl no-print">
                <h4 className="font-bold text-primary-600 border-b border-primary-600/20 pb-3 mb-4 uppercase text-xs tracking-wider flex items-center gap-2">
                ...
                <div className="space-y-4">
                  {request.items.map((item) => {
                    ...
                    const approvedQuantity =
                      itemStatus?.approvedQuantity ?? item.quantity;
                    ...
                    if (isStockAllocated) {
                      return (
                        <div
                          key={item.id}
                          className="p-4 bg-white border border-emerald-200 rounded-lg shadow-sm flex items-center justify-between"
                        >
                    ...
                    return (
                      <ItemPurchaseDetailsForm
                        ...
                        onChange={(details) =>
                          handlePurchaseDetailChange(item.id, details)
                        }
                        isDisabled={false}
                      />
                    );
                ...
                <div className="mt-6 flex justify-end items-center border-t border-primary-600/20 pt-4">
                  <div className="text-right">
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
                      Estimasi Total Keseluruhan
                    </p>
                    <p className="text-2xl font-mono font-black text-primary-600">
          ...
          {request.isRegistered && (
            <section className="p-5 bg-emerald-50 border border-emerald-200 rounded-xl shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {assets
                  .filter((a) => a.woRoIntNumber === request.id)
                  .map((asset) => (
                    <ClickableLink
                      key={asset.id}
                      onClick={() =>
                      ...
                      className="bg-white px-3 py-1.5 rounded-lg border border-emerald-200 text-xs font-bold text-emerald-700 hover:bg-emerald-100 transition-colors shadow-sm hover:shadow-md"
                    >
                      {asset.id}
                    </ClickableLink>
                  ))}
        ...
        {showProcurement && (
          <ProcurementProgressCard request={request} assets={assets} />
        )}
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-start gap-4">
              <Avatar
                ...
                className="w-10 h-10 flex-shrink-0 text-sm shadow-sm border border-slate-200"
              />
              <div className="flex-1">
                ...
                <div className="relative group">
                  <textarea
                    ...
                    className="block w-full px-4 py-3 pr-12 text-sm text-slate-700 placeholder:text-slate-400 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 resize-none transition-all disabled:bg-slate-100 disabled:text-slate-400 shadow-inner"
                    placeholder={
                      isCommentDisabled
                        ? "Diskusi ditutup."
                        : "Tulis komentar..."
                  ...
                  {!isCommentDisabled && (
                    <button
                      ...
                      className="absolute bottom-1.5 right-1.5 p-2 text-white bg-primary-600 rounded-full shadow-md hover:bg-primary-700 disabled:bg-slate-300 disabled:shadow-none transition-all transform active:scale-95"
                    >
            ...
            <div className="border-t border-slate-100 pt-6">
              <CommentThread
                activities={(request.activityLog || [])
                  .filter((a) => !a.parentId)
                  .sort(
                    (a, b) =>
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Permintaan yang dibatalkan tidak dapat diproses lebih lanjut.
          ...
          footerContent={
            <div className="flex gap-2 w-full justify-end">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-bold text-white bg-red-600 rounded-lg shadow-sm hover:bg-red-700"
              >
                Ya, Hapus
              </button>
            </div>
