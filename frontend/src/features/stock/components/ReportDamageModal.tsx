...
const ReportDamageModal: React.FC<ReportDamageModalProps> = ({
  ...
  const conditionOptions = [
    {
      value: AssetCondition.MINOR_DAMAGE,
      label: "Rusak Ringan (Perlu Perbaikan)",
    },
  ...
  const handleSubmit = () => {
    ...
    const processedAttachments: Attachment[] = attachments.map(
      (file, index) => ({
        id: Date.now() + index,
        name: file.name,
      ...
      footerContent={
        <>
          ...
          <button
            ...
            disabled={isLoading}
            className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-danger rounded-lg shadow-sm hover:bg-red-700 disabled:bg-red-300"
          >
          ...
          <div className="flex items-center justify-center w-full px-6 pt-5 pb-6 mt-1 border-2 border-gray-300 border-dashed rounded-md">
            <div className="space-y-1 text-center">
              ...
              <div className="flex text-sm text-gray-600">
                <label
                  htmlFor="file-upload-damage"
                  className="relative font-medium bg-white rounded-md cursor-pointer text-primary-600 hover:text-primary-500 focus-within:outline-none"
                >
                  <span>Pilih file</span>
                  <input
                    id="file-upload-damage"
                    name="file-upload-damage"
                    type="file"
                    className="sr-only"
                    multiple
