...
const InfoItem: React.FC<{
  ...
  <div className={className}>
    ...
    <dd className="mt-1 text-sm font-semibold text-gray-800 break-words">
      {children}
    </dd>
  </div>
);
...
export const AssetCard: React.FC<{
  ...
  const isCutPiece =
    asset.name.includes("(Potongan)") || asset.id.includes("-PART-");
  ...
  return (
    <div className="flex flex-col bg-white border border-gray-200/80 rounded-xl shadow-sm transition-all duration-300 hover:shadow-lg hover:border-primary-500/50 group">
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {isMeasurement && (
                <span
                  className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${isCutPiece ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-indigo-50 text-indigo-700 border-indigo-200"}`}
                >
                  {isCutPiece ? (
                    <BsScissors className="w-3 h-3" />
                  ) : (
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              ...
              <span
                className={`px-2 py-0.5 text-[10px] font-semibold rounded-full border whitespace-nowrap ${getStatusClass(asset.status)}`}
              >
                {asset.status}
              </span>
            </div>
        ...
        <p className="flex items-center gap-2 mt-2 text-xs font-mono text-gray-500 bg-gray-50 px-2 py-1 rounded w-fit">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              copyToClipboard(asset.id, "ID Aset");
            }}
            title="Salin ID Aset"
            ...
            <div className="flex justify-between items-end mb-1">
              ...
              <div className="text-right">
                <span className="text-lg font-bold text-gray-800">
                  {currentBal.toLocaleString("id-ID")}
                </span>
                <span className="text-xs text-gray-500 ml-1">
                  / {initialBal.toLocaleString("id-ID")} {unitLabel}
                </span>
              </div>
            </div>
              ...
              {asset.serialNumber ? (
                <span className="flex items-center gap-1.5 font-mono text-xs">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      copyToClipboard(asset.serialNumber!, "Nomor Seri");
                    }}
                    title="Salin SN"
      ...
      <div className="grid grid-cols-2 gap-3 p-3 bg-gray-50 border-t rounded-b-xl">
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 text-xs font-bold text-gray-700 transition-colors bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-100 hover:text-primary-600"
        >
