...
export const AssetStatusDonutChart: React.FC<DonutChartProps> = ({
  assets,
}) => {
  const data = useMemo(() => {
    const statusCounts = {
    ...
    assets.forEach((a) => {
      ...
      else if (a.status === AssetStatus.IN_STORAGE)
        statusCounts[AssetStatus.IN_STORAGE]++;
      else if (a.status === AssetStatus.DAMAGED)
        statusCounts[AssetStatus.DAMAGED]++;
    ...
    return [
      {
        label: "Digunakan",
        value: statusCounts[AssetStatus.IN_USE],
        color: "#3B82F6",
        percent: (statusCounts[AssetStatus.IN_USE] / total) * 100,
      },
      {
        label: "Disimpan",
        value: statusCounts[AssetStatus.IN_STORAGE],
        color: "#10B981",
        percent: (statusCounts[AssetStatus.IN_STORAGE] / total) * 100,
      },
      {
        label: "Rusak",
        value: statusCounts[AssetStatus.DAMAGED],
        color: "#EF4444",
        percent: (statusCounts[AssetStatus.DAMAGED] / total) * 100,
      },
      {
        label: "Lainnya",
        value: statusCounts.others,
        color: "#F59E0B",
        percent: (statusCounts.others / total) * 100,
      },
        ...
        return (
          <div key={i} className="group">
            <div className="flex justify-between text-xs mb-1.5">
              <span
                className="font-semibold text-gray-600 truncate max-w-[180px] group-hover:text-primary-600 transition-colors"
                title={item.label}
              >
                {item.label}
              </span>
            ...
            <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden border border-gray-100/50">
              <div
                className="h-full bg-gradient-to-r from-primary-600 to-blue-400 rounded-full transition-all duration-1000 ease-out relative"
...
export const TechnicianLeaderboard: React.FC<TechLeaderboardProps> = ({
  data,
}) => {
