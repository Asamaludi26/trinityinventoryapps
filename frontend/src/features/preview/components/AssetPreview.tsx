...
const SmartContextCard: React.FC<{
  ...
  if (
    ...
    asset.currentUser?.startsWith("CUST-")
  ) {
    ...
    const installationDoc = installations.find(
      (i) =>
        i.customerId === asset.currentUser &&
        (i.assetsInstalled.some((ai) => ai.assetId === asset.id) ||
          i.materialsUsed?.some((mi) => mi.materialAssetId === asset.id)),
    );
          ...
          <div className="flex-1">
            ...
            {customer ? (
              <div className="mt-1 text-sm text-blue-800">
                ...
                <div className="mt-2 flex gap-2">
                  <button
                    ...
                    className="text-xs bg-white px-2 py-1 rounded border border-blue-200 hover:bg-blue-100 font-medium transition-colors"
                  >
                  ...
                  {installationDoc && (
                    <button
                      ...
                      className="text-xs bg-white px-2 py-1 rounded border border-blue-200 hover:bg-blue-100 font-medium transition-colors flex items-center gap-1"
                    >
              ...
              <p className="text-xs text-red-500 mt-1">
                Data pelanggan ({asset.currentUser}) tidak ditemukan.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 text-sm text-indigo-800">
              ...
              <p className="text-xs mt-1 text-indigo-600 italic">
                Lokasi: {asset.location || "Tidak spesifik"}
              </p>
            </div>
          </div>
        </div>
  ...
  if (
    ...
    const activeMnt = maintenances.find(
      (m) =>
        m.status !== "Selesai" &&
        (m.assets?.some((a) => a.assetId === asset.id) ||
          m.problemDescription.includes(asset.name)),
    );
    ...
    return (
      <div
        className={`p-4 rounded-xl bg-${colorClass}-50 border border-${colorClass}-200 shadow-sm animate-fade-in-up`}
      >
        <div className="flex items-start gap-3">
          <div
            className={`p-2 bg-${colorClass}-100 text-${colorClass}-600 rounded-lg`}
          >
          ...
          <div className="flex-1">
            ...
            {activeMnt ? (
              <div className={`mt-2 text-xs text-${colorClass}-800`}>
                <p>
                  <strong>Tiket:</strong> {activeMnt.docNumber}
                </p>
                <p>
                  <strong>Teknisi:</strong> {activeMnt.technician}
                </p>
                <p className="mt-1 italic line-clamp-1">
                  "{activeMnt.problemDescription}"
                </p>
                <button
                  onClick={() =>
                    onShowPreview({ type: "maintenance", id: activeMnt.id })
                  }
                  className="mt-2 text-xs bg-white px-2 py-1 rounded border hover:opacity-80 font-medium transition-colors flex items-center gap-1"
                >
                  <BsFileEarmarkText /> Lihat Laporan
                </button>
              </div>
            ) : (
              <p className={`text-xs mt-1 text-${colorClass}-700 italic`}>
                Belum ada tiket maintenance aktif.
              </p>
            )}
          </div>
        </div>
  ...
  if (asset.status === AssetStatus.IN_STORAGE && showSensitiveData) {
    ...
    const lastActivity = [...asset.activityLog]
      .reverse()
      .find((a) => a.action.includes("Status") || a.action.includes("Dicatat"));
    const lastDate = lastActivity
      ? new Date(lastActivity.timestamp)
      : new Date(asset.registrationDate);
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 mt-1">
              Lokasi: <span className="font-semibold">{asset.location}</span>{" "}
              {asset.locationDetail && `(${asset.locationDetail})`}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Aging: <strong>{daysInStorage} hari</strong> sejak aktivitas
              terakhir.
            </p>
          </div>
        </div>
    ...
    <div className="p-4 mt-4 bg-green-50 border border-green-200 rounded-lg">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-bold text-green-800">
          Estimasi Nilai Aset (Depresiasi)
        </h3>
      </div>
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <p className="font-semibold text-gray-800">
            Rp {depreciation.initialValue.toLocaleString("id-ID")}
          </p>
        </div>
        <div>
          <p className="text-xs text-green-600">Nilai Buku Saat Ini</p>
          <p className="font-bold text-green-800 text-lg">
            Rp {depreciation.currentValue.toLocaleString("id-ID")}
          </p>
        </div>
        <div>
          <p className="text-xs text-green-600">Umur Ekonomis</p>
          <p className="text-gray-800">
            {depreciation.usefulLifeYears} Tahun ({depreciation.monthsPassed}{" "}
            bulan berjalan)
          </p>
        </div>
        <div>
          ...
          <p className="text-gray-800">
            Rp {depreciation.monthlyDepreciation.toLocaleString("id-ID")}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="mt-8 border-t pt-6 bg-gray-50/50 rounded-xl p-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Gunakan untuk identifikasi fisik aset.
          </p>
        </div>
      ...
      <p className="text-center text-[10px] text-gray-400 mt-2 no-print">
        Tips: Gunakan kertas stiker label ukuran 70mm x 40mm atau cetak pada
        kertas A4 lalu gunting.
      </p>
    </div>
  );
};
...
const getLogStyle = (action: string) => {
  if (action.includes("Dicatat") || action.includes("Baru"))
    return {
      icon: RegisterIcon,
      bg: "bg-green-100",
      text: "text-green-600",
  ...
  if (action.includes("Serah Terima") || action.includes("Handover"))
    return {
      icon: HandoverIcon,
      bg: "bg-blue-100",
      text: "text-blue-600",
...
export const AssetPreview: React.FC<AssetPreviewProps> = ({
  ...
  const stockStats = useMemo(() => {
    const sameModelAssets = allAssets.filter(
      (a) => a.name === asset.name && a.brand === asset.brand,
    );
    const inStorageCount = sameModelAssets.filter(
    ...
    const isMeasurement =
      asset.initialBalance !== undefined && asset.currentBalance !== undefined;
  ...
  const measurementPercent = useMemo(() => {
    if (
      ...
      return Math.round(
        ((asset.currentBalance || 0) / asset.initialBalance) * 100,
      );
    }
    return 0;
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
          <button
            ...
            className={`py-3 px-1 border-b-2 text-sm font-medium ${activeTab === "details" ? "border-primary-600 text-primary-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
          >
            Detail
          </button>
          <button
            onClick={() => setActiveTab("history")}
            className={`py-3 px-1 border-b-2 text-sm font-medium ${activeTab === "history" ? "border-primary-600 text-primary-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
          >
            Riwayat
          </button>
          <button
            onClick={() => setActiveTab("attachments")}
            className={`py-3 px-1 border-b-2 text-sm font-medium ${activeTab === "attachments" ? "border-primary-600 text-primary-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
          >
            Lampiran
          </button>
        </nav>
        ...
        {activeTab === "details" && (
          <div className="space-y-6">
            ...
            <SmartContextCard
              ...
              showSensitiveData={isOpsAdmin}
            />

            {/* STOCK INFO CARD - ONLY FOR OPS ADMIN & NOT FOR POTONGAN */}
            {isOpsAdmin && !isPotongan && (
              <div
                className={`flex items-start gap-4 p-4 rounded-xl border ${stockStats.inStorage > 0 ? "bg-blue-50/50 border-blue-200" : "bg-red-50/50 border-red-200"}`}
              >
                <div
                  className={`p-2 rounded-full ${stockStats.inStorage > 0 ? "bg-blue-100 text-blue-600" : "bg-red-100 text-red-600"}`}
                >
                  <ArchiveBoxIcon className="w-5 h-5" />
                </div>
                <div className="flex-1">
                  <h4
                    className={`text-sm font-bold ${stockStats.inStorage > 0 ? "text-blue-800" : "text-red-800"}`}
                  >
                    Statistik Stok Gudang
                  </h4>
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-6 mt-1">
                    <div className="text-xs text-gray-600">
                      Unit Sejenis Tersedia:{" "}
                      <strong className="text-gray-900">
                        {stockStats.inStorage}
                      </strong>{" "}
                      / {stockStats.total}
                    </div>
                    {stockStats.isMeasurement && (
                      <div className="flex items-center gap-1.5 text-xs text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                        ...
                        <span>
                          Sisa Isi Unit Ini:{" "}
                          <strong>
                            {asset.currentBalance?.toLocaleString("id-ID")}
                          </strong>{" "}
                          (Awal: {asset.initialBalance?.toLocaleString("id-ID")}
                          )
                        </span>
                      </div>
                    )}
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-end text-sm">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-gray-500 text-xs ml-1">
                        / {asset.initialBalance?.toLocaleString("id-ID")}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden border border-gray-200">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${
                        measurementPercent < 20
                          ? "bg-red-500"
                          : measurementPercent < 50
                            ? "bg-amber-400"
                            : "bg-green-500"
                      }`}
                  ...
                  <p className="text-xs text-gray-400 text-center italic">
                    {measurementPercent < 20
                      ? "Unit ini hampir habis. Harap lapor jika perlu restock."
                      : "Kondisi isi masih mencukupi."}
                  </p>
                </div>
              </div>
            )}
                ...
                <PreviewRow label="MAC Address">
                  <div className="flex items-center gap-2 font-mono">
                    ...
                    {asset.macAddress && (
                      <button
                        onClick={() =>
                          navigator.clipboard.writeText(asset.macAddress!)
                        }
                        title="Salin"
                        className="text-gray-400 hover:text-primary-600"
                      >
            ...
            <div>
              ...
              <dl className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                <PreviewRow
                  ...
                  value={
                    <span
                      className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getAssetStatusClass(asset.status as AssetStatus)}`}
                    >
                      {asset.status}
                    </span>
                  }
          ...
          <div className="relative border-l-2 border-gray-200 ml-4 space-y-8">
            {asset.activityLog
              ...
              .map((log) => {
                ...
                const linkType = log.referenceId
                  ? getLinkTarget(log.referenceId)
                  : null;
                ...
                return (
                  <div key={log.id} className="relative ml-6">
                    {/* Timeline Dot */}
                    <div
                      className={`absolute -left-[37px] top-1 flex h-8 w-8 items-center justify-center rounded-full border-2 bg-white ${border} ${text}`}
                    >
            ...
            {asset.attachments.length > 0 ? (
              asset.attachments.map((att) => (
                ...
                >
                  <div className="aspect-w-16 aspect-h-10 bg-gray-100 flex items-center justify-center overflow-hidden">
                    {att.type === "image" ? (
                      <img
                        src={att.url}
                        alt={att.name}
                        className="object-cover w-full h-32 group-hover:scale-105 transition-transform duration-500"
                      />
                    ) : (
                  ...
                  <div className="p-3">
                    <p
                      className="text-xs font-semibold text-gray-800 truncate mb-2"
                      title={att.name}
                    >
                      {att.name}
                    </p>
                    <div className="flex gap-2">
                      <a
                        ...
                        className="flex-1 flex items-center justify-center gap-1 py-1.5 text-[10px] font-bold text-gray-600 bg-gray-50 rounded hover:bg-gray-100 hover:text-primary-600 transition-colors border border-gray-200"
                      >
