...
const FormSection: React.FC<{
  ...
  >
    ...
    <div className="p-6 grid grid-cols-1 gap-6 md:grid-cols-2">{children}</div>
  </div>
);
...
const CustomerForm: React.FC<CustomerFormProps> = ({
  ...
  const [initialAssignedAssetIds, setInitialAssignedAssetIds] = useState<
    string[]
  >([]);
  ...
  const availableAssets = useMemo(() => {
    return installableAssets.filter(
      (opt) => !assignedAssetIds.includes(opt.value),
    );
    ...
    if (customer) {
      ...
      if (materialOptions.length > 0) {
        standardMaterialKeywords.forEach((keyword) => {
          const match = materialOptions.find((opt) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newlyAssigned = assignedAssetIds.filter(
        (id) => !initialAssignedAssetIds.includes(id),
      );
      const unassigned = initialAssignedAssetIds.filter(
      ...
      const finalMaterials = materials
        ...
        .map((m) => {
          ...
          return {
            ...
            unit: m.unit,
            installationDate:
              customer?.installedMaterials?.find(
                (em) => `${em.itemName}|${em.brand}` === m.modelKey,
  ...
  const ActionButtons: React.FC<{ formId?: string }> = ({ formId }) => (
    <>
      <button
        type="button"
        onClick={onCancel}
        className="px-4 py-2 text-sm font-bold text-gray-700 bg-white border border-gray-300 rounded-xl shadow-sm hover:bg-gray-50 transition-all"
      >
        Batal
      </button>
      <button
        ...
        className={`inline-flex items-center justify-center px-6 py-2.5 text-sm font-bold text-white transition-all duration-200 rounded-xl shadow-lg 
                ${isLoading || !isFormValid ? "bg-gray-300 cursor-not-allowed shadow-none" : "bg-primary-600 hover:bg-primary-700 hover:-translate-y-0.5"}`}
      >
    ...
    <>
      <form id={formId} onSubmit={handleSubmit} className="space-y-6 pb-32">
        ...
        <FormSection
          ...
          subtitle="Informasi dasar dan kontak."
          icon={<UsersIcon className="w-5 h-5" />}
        >
          <div className="md:col-span-2">
            <label
              htmlFor="customerId"
              className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1"
            >
            ...
            <div className="relative">
              <input
                ...
                disabled={!!customer}
                required
                className={`block w-full px-4 py-2.5 text-gray-900 bg-white border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm font-mono uppercase transition-all ${idError ? "border-red-300 bg-red-50" : isIdAvailable ? "border-green-300" : "border-gray-300"} ${customer ? "bg-gray-100 text-gray-500 cursor-not-allowed" : ""}`}
                placeholder="Contoh: CUST-001"
              />
              {!customer && isIdAvailable && (
                <CheckIcon className="absolute right-3 top-3 w-5 h-5 text-green-500" />
              )}
            </div>
            {idError && (
              <p className="mt-1.5 text-xs text-red-600 font-medium flex items-center gap-1">
                <TrashIcon className="w-3 h-3" /> {idError}
              </p>
            )}
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              required
              className="block w-full px-4 py-2.5 text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm transition-all"
            />
          </div>
          <div>
            <label
            ...
            <input
              ...
              required
              className="block w-full px-4 py-2.5 text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm font-mono"
              placeholder="+62-..."
            />
          </div>
          <div>
            ...
            <input
              ...
              onBlur={validateForm}
              className={`block w-full px-4 py-2.5 text-gray-900 bg-white border rounded-lg focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm ${emailError ? "border-red-300" : "border-gray-300"}`}
            />
            {emailError && (
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              required
              rows={3}
              className={`block w-full px-4 py-2.5 text-gray-900 bg-white border rounded-lg focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm ${addressError ? "border-red-300" : "border-gray-300"}`}
        ...
        <FormSection
          ...
          subtitle="Paket internet dan status berlangganan."
          icon={<WrenchIcon className="w-5 h-5" />}
        >
          <div>
            <label
              htmlFor="customerPackage"
              className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1"
            >
              Paket Bandwidth
            </label>
            <div className="relative">
              <input
                ...
                placeholder="50"
                required
                className="block w-full px-4 py-2.5 pr-12 text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm font-bold"
              />
        ...
        <FormSection
          ...
          subtitle="Modem, Router, dan perangkat aktif lainnya."
            ...
            <div className="space-y-2">
              {assignedAssetIds.map((assetId) => {
                ...
                return (
                  <div
                    key={assetId}
                    className="flex items-center justify-between p-3 bg-blue-50/50 border border-blue-100 rounded-xl group hover:border-blue-300 transition-all"
                  >
        ...
        <FormSection
          ...
          subtitle="Kabel, konektor, dan aksesoris pasif."
          ...
          <div className="md:col-span-2">
            <div className="space-y-3">
              {materials.map((material, index) => (
                <div
                  key={material.tempId}
                  className="flex flex-col sm:flex-row gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl items-start sm:items-center animate-fade-in-up"
                >
                  <div className="flex-1 w-full">
                    ...
                    <CustomSelect
                      ...
                      onChange={(value) =>
                        handleMaterialChange(material.tempId, "modelKey", value)
                      }
                      isSearchable
                  ...
                  <div className="w-full sm:w-32">
                    ...
                    <div className="relative">
                      <input
                        ...
                        min="0"
                        className="block w-full px-3 py-2 text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm font-bold"
                      />
                      <span className="absolute right-3 top-2 text-xs font-medium text-gray-500 pointer-events-none">
                        {material.unit}
                      </span>
                    </div>
                  </div>
                  <div className="w-full sm:w-auto flex items-end gap-2">
                    <div className="flex-1 sm:flex-none">
                      <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block opacity-0 sm:opacity-100">
                        Sumber
                      </label>
                      <button
                        type="button"
                    ...
                    <button
                      ...
                      className="h-[38px] w-[38px] flex items-center justify-center text-gray-400 bg-white border border-gray-200 rounded-lg hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-colors mb-[1px]"
                    >
                      <TrashIcon className="w-4 h-4" />
                    </button>
                  </div>
            ...
            <button
              ...
              className="mt-4 w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 font-bold text-xs hover:border-primary-600 hover:text-primary-600 hover:bg-blue-50/50 transition-all flex items-center justify-center gap-2"
            >
        ...
        <FormSection
          title="Catatan & Lampiran"
          subtitle="Info tambahan dan dokumen pendukung."
        ...
        >
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              className="block w-full px-3 py-2 text-gray-900 placeholder:text-gray-400 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-600/20 focus:border-primary-600 text-sm"
          ...
          <div className="md:col-span-2">
            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1">
              Upload Lampiran (Foto Rumah / KTP)
            </label>
            <div
              ...
              className={`flex flex-col items-center justify-center w-full px-6 pt-5 pb-6 mt-1 border-2 border-dashed rounded-lg transition-colors cursor-pointer
                                ${isDragging ? "border-primary-600 bg-blue-50" : "border-gray-300 hover:bg-gray-50"}`}
            >
            ...
            {customer &&
              ...
              customer.attachments.length > 0 && (
                <p className="text-xs text-gray-500 mt-2 italic">
                  * Pelanggan ini sudah memiliki {customer.attachments.length}{" "}
                  lampiran sebelumnya. File baru akan ditambahkan.
                </p>
              )}
          </div>
        </FormSection>
        ...
        <div
          ref={footerRef}
          className="flex justify-end pt-5 mt-5 space-x-3 border-t"
        >
