...
const AssetCard: React.FC<{
  ...
  const ageInMonths = Math.floor(
    (new Date().getTime() - installDate.getTime()) / (1000 * 60 * 60 * 24 * 30),
  );
  const isOld = ageInMonths > 36; // 3 Tahun

  return (
    <div className="group flex flex-col bg-white border border-gray-200 rounded-xl shadow-sm hover:border-primary-600/50 transition-all overflow-hidden">
      <div className="p-4 flex items-start justify-between bg-gray-50/50 border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-lg ${isOld ? "bg-amber-100 text-amber-600" : "bg-blue-100 text-blue-600"}`}
          >
            <BsRouter className="w-5 h-5" />
          </div>
          <div>
            <h4
              className="text-sm font-bold text-gray-800 group-hover:text-primary-600 transition-colors cursor-pointer"
        ...
        <span
          className={`px-2 py-0.5 text-[10px] font-bold rounded-full border ${asset.status === AssetStatus.DAMAGED ? "bg-red-50 text-red-600 border-red-100" : "bg-green-50 text-green-600 border-green-100"}`}
        >
          {asset.status}
        </span>
      </div>
      <div className="p-4 flex-1">
        <div className="grid grid-cols-2 gap-y-2 text-xs text-gray-600 mb-3">
          ...
          <div className="flex flex-col">
            <span className="text-gray-400">Usia Pakai</span>
            <span className={`font-medium ${isOld ? "text-amber-600" : ""}`}>
              {ageInMonths} Bulan
            </span>
          </div>
        </div>
      ...
      <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
        {!isSuspended && (
          <button
            ...
            className="p-1.5 text-amber-600 bg-white border border-amber-200 rounded hover:bg-amber-50"
        ...
        <button
          onClick={() => onDismantle(asset.id)}
          className="p-1.5 text-red-600 bg-white border border-red-200 rounded hover:bg-red-50"
          title="Dismantle / Tarik"
        >
      ...
      {activities.map((act, idx) => {
        ...
        return (
          <div key={idx} className="relative ml-6">
            <div
              className={`absolute -left-[37px] top-0 flex h-8 w-8 items-center justify-center rounded-full border-2 bg-white ${color}`}
            >
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start bg-gray-50 p-4 rounded-xl border border-gray-100 hover:border-blue-200 transition-colors">
              ...
              <div className="mt-2 sm:mt-0 text-right">
                <p className="text-xs font-bold text-gray-500">
                  {act.date.toLocaleDateString("id-ID")}
                </p>
                    ...
                    onClick={() => {
                      if (act.type === "installation")
                        setActivePage("customer-installation-form", {
                          openDetailForId: act.id,
                        });
                      if (act.type === "maintenance")
                        setActivePage("customer-maintenance-form", {
                          openDetailForId: act.id,
                        });
                      if (act.type === "dismantle")
                        setActivePage("customer-dismantle", {
                          openDetailForId: act.id,
                        });
                    }}
...
const CustomerDetailPage: React.FC<CustomerDetailPageProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "assets" | "documents" | "history"
  >("overview");
  ...
  const customer = useMemo(
    () => customers.find((c) => c.id === initialState.customerId),
    [customers, initialState.customerId],
  );
  const customerAssets = useMemo(
    () => assets.filter((a) => a.currentUser === initialState.customerId),
    [assets, initialState.customerId],
  );
  ...
  const {
    ...
    customerAssets.forEach((a) => {
      ...
      if (
        a.serialNumber &&
        !a.name.toLowerCase().includes("kabel") &&
        !a.name.toLowerCase().includes("dropcore")
      ) {
        active.push(a);
      }
  ...
  const healthStatus = useMemo(() => {
    ...
    if (openTicketCount > 0)
      return {
        status: `PERLU PERHATIAN (${openTicketCount} Tiket)`,
  ...
  const handleNewDismantle = () =>
    setActivePage("customer-dismantle", { prefillCustomerId: customer.id });
  ...
  const renderActionButtons = () => (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        title={
          !hasInstalledAssets
            ? "Tidak dapat maintenance: Belum ada aset/material terpasang (Status IN_USE)."
      ...
      <button
        ...
        title={
          !hasInstalledAssets
            ? "Tidak dapat dismantle: Belum ada aset/material terpasang (Status IN_USE)."
            : isInactive
              ? "Pelanggan Non-Aktif: Segera Tarik Aset"
              : "Tarik Aset (Dismantle)"
        }
      >
        <DismantleIcon className="w-4 h-4" />{" "}
        {isInactive ? "Tarik Segera" : "Tarik Aset"}
      </button>
      ...
      <button
        ...
        className="inline-flex items-center justify-center gap-2 px-3 py-2 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 transition-all"
      >
  ...
  const OverviewTab = () => (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        <div
          className={`bg-white p-4 rounded-xl border flex flex-col justify-center items-center text-center shadow-sm ${healthStatus.color.replace("text-", "border-").split(" ")[2]}`}
        >
          <p className="text-xs font-bold uppercase tracking-wider opacity-70">
            Status Kesehatan
          </p>
          <span
            className={`mt-2 px-3 py-1 rounded-full text-xs font-black uppercase ${healthStatus.color}`}
          >
            {healthStatus.status}
          </span>
        </div>
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">
              Perangkat Terpasang ({activeDevices.length})
            </h3>
            <button
          ...
          {activeDevices.length > 0 ? (
            ...
            <div className="text-center py-10 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
              ...
              <button
                onClick={handleNewInstallation}
                className="mt-3 text-xs font-bold text-primary-600 border border-primary-600 px-3 py-1.5 rounded hover:bg-blue-50 transition-colors"
              >
    ...
    <div className="space-y-8 animate-fade-in-up">
      ...
      <section>
        <div className="flex items-center justify-between gap-3 mb-4">
          ...
          <button
            onClick={handleNewInstallation}
            className="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-bold text-primary-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition-colors"
          >
      ...
      <section>
        <div className="flex items-center gap-3 mb-4 pt-6 border-t border-gray-200">
          <div className="p-2 bg-orange-100 text-orange-600 rounded-lg">
            <BsHddNetwork className="w-5 h-5" />
          </div>
          <h3 className="text-lg font-bold text-gray-800">
                ...
                {materialHistory.map((log, idx) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase border ${log.type === "installation" ? "bg-blue-50 text-blue-600 border-blue-100" : "bg-orange-50 text-orange-600 border-orange-100"}`}
                        >
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        ...
        <div className="space-y-2">
          {[...installations, ...maintenances, ...dismantles]
            ...
            .sort((a, b) => {
              const dateA =
                "installationDate" in a
                  ? a.installationDate
                  : "maintenanceDate" in a
                    ? a.maintenanceDate
                    : a.dismantleDate;
              const dateB =
                "installationDate" in b
                  ? b.installationDate
                  : "maintenanceDate" in b
                    ? b.maintenanceDate
                    : b.dismantleDate;
            ...
            .map((doc: any) => (
              <div
                key={doc.id}
                className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100 hover:border-blue-300 transition-colors cursor-pointer"
                onClick={() =>
                  onShowPreview({
                    type: doc.docNumber.startsWith("WO-MT")
                      ? "maintenance"
                      : doc.docNumber.startsWith("WO-DSM")
                        ? "dismantle"
                        : "installation",
                    id: doc.id,
                  })
          ...
          {[...installations, ...maintenances, ...dismantles].filter(
        ...
        {customer.attachments && customer.attachments.length > 0 ? (
          <div className="grid grid-cols-2 gap-3">
            {customer.attachments.map((att) => (
              <div
                key={att.id}
                className="group relative bg-gray-50 border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-all"
              >
                ...
                <div className="p-2">
                  ...
                  <div className="flex gap-1 mt-2">
                    <button
                      ...
                      className="flex-1 py-1 bg-white border rounded text-[10px] font-bold text-blue-600 hover:bg-blue-50"
                    >
                      Lihat
                    </button>
                    <a
                      href={att.url}
                      download={att.name}
                      className="flex-1 py-1 bg-white border rounded text-[10px] font-bold text-gray-600 hover:bg-gray-50 text-center"
                    >
                      <DownloadIcon className="w-3 h-3 inline" />
                    </a>
                  </div>
                </div>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-lg">
                Pelanggan Ditangguhkan (Suspend)
              </p>
              <p className="text-sm opacity-90">
                Layanan dihentikan sementara. Tidak dapat melakukan instalasi
                atau maintenance baru.
              </p>
            </div>
          </div>
        </div>
