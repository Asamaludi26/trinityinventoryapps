...
const DismantleForm: React.FC<DismantleFormProps> = ({
  currentUser,
  dismantles,
  onSave,
  onCancel,
  ...
  const [retrievedCondition, setRetrievedCondition] = useState<AssetCondition>(
  ...
  const selectedAsset = useMemo(
    () => assetsForCustomer.find((a) => a.id === selectedAssetId) || null,
    [assetsForCustomer, selectedAssetId],
  );
            ...
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                <h4 className="font-semibold text-gray-800 text-sm flex items-center gap-2">
                  <BsBoxSeam className="text-blue-600" /> Perangkat Terpasang
                  (Pilih untuk ditarik)
                </h4>
                <span className="text-xs text-gray-500">
                  {assetsForCustomer.length} Unit
                </span>
              </div>
        ...
        <div className="p-4 border-t border-b border-gray-200">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <CustomSelect
                options={Object.values(AssetCondition).map((c) => ({
                  value: c,
                  label: c,
                }))}
                value={retrievedCondition}
                onChange={(v) => setRetrievedCondition(v as AssetCondition)}
              />
            </div>
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={3}
                className="block w-full px-3 py-2 mt-1 text-gray-900 placeholder:text-gray-400 bg-gray-50 border border-gray-300 rounded-lg shadow-sm sm:text-sm"
                placeholder="Contoh: Unit ditarik karena pelanggan upgrade, kondisi fisik baik..."
              ></textarea>
            </div>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Lampiran (Foto Kondisi, BAST Fisik)
              </label>
              ...
              <div className="flex items-center justify-center mt-2 h-28">
                {technician && (
                  <SignatureStamp
                    ...
                    signatureDate={dismantleDate?.toISOString() || ""}
                  />
                )}
              </div>
              <p className="pt-1 mt-2 text-sm text-center text-gray-600">
                ( {technician || "Nama Jelas"} )
              </p>
            </div>
          </div>
        </div>
