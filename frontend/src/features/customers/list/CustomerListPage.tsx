                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                             <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-sm font-semibold tracking-wider text-left text-gray-500">Aset & Material</th> {/* Ubah Header */}
                                    ...
                                    <tr key={customer.id} {...longPressHandlers} onClick={() => isBulkSelectMode ? setSelectedCustomerIds(prev => prev.includes(customer.id) ? prev.filter(id => id !== customer.id) : [...prev, customer.id]) : onShowPreview({ type: 'customer', id: customer.id })} className={`cursor-pointer transition-colors ${selectedCustomerIds.includes(customer.id) ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap align-top">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); onShowPreview({type: 'customerAssets', id: customer.id})}} 
                                                    className="flex items-center gap-2 text-sm font-bold text-gray-800 hover:text-tm-primary transition-colors text-left"
                                                ...
                                                {materialCount > 0 && (
                                                    <div className="flex items-center gap-2 text-xs text-gray-400 pl-0.5" title="Material habis pakai (Kabel, Konektor, dll)">
