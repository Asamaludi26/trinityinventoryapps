...
const MaintenanceDetailPage: React.FC<MaintenanceDetailPageProps> = ({
  maintenance,
  currentUser,
  assets,
  onBackToList,
  ...
  const canComplete =
    maintenance.status === ItemStatus.IN_PROGRESS &&
    ...
    <>
      {canComplete && (
        <button
          onClick={onComplete}
          disabled={isLoading}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-success rounded-lg shadow-sm hover:bg-green-700"
        >
  ...
  return (
    <DetailPageLayout
      title={`Laporan Maintenance: ${maintenance.docNumber}`}
      onBack={onBackToList}
      ...
      <div className="p-8 bg-white border rounded-lg shadow-sm">
        ...
        <div className="text-center my-8">
          ...
          <p className="text-sm text-gray-500">
            Dokumen : {maintenance.docNumber}
          </p>
        </div>

        <section className="space-y-4 text-sm mb-6">
          <div className="grid grid-cols-2 gap-6">
            <DetailItem label="Tanggal Kunjungan">
            ...
            <DetailItem label="Nomor Dokumen">
              {maintenance.docNumber}
            </DetailItem>
            <DetailItem label="Nomor Request Terkait">
              {maintenance.requestNumber || "-"}
            </DetailItem>
            <DetailItem label="Pelanggan">
              {maintenance.customerName}
            </DetailItem>
            <DetailItem label="ID Pelanggan">
              {maintenance.customerId}
            </DetailItem>
            <DetailItem label="Prioritas">
              <span
                className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getPriorityClass(maintenance.priority)}`}
              >
                ...
                <DetailItem label="Lingkup Pekerjaan">
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    {maintenance.workTypes.map((wt) => (
                      <span
                        key={wt}
                        className="px-2.5 py-1 text-xs font-semibold text-primary-600 bg-blue-100 rounded-full"
                      >
                        {wt}
                      </span>
                    ))}
              ...
              <div className="overflow-x-auto border rounded-lg">
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-100">
                    <tr>
                      <th className="p-3 font-semibold text-left text-gray-600 w-12">
                        No.
                      </th>
                      <th className="p-3 font-semibold text-left text-gray-600">
                        Nama Aset
                      </th>
                      <th className="p-3 font-semibold text-left text-gray-600">
                        ID Aset
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {maintenance.assets.map((mAsset, index) => {
                          ...
                          <tr key={mAsset.assetId}>
                            ...
                            <td className="p-3" colSpan={5}>
                              ...
                              <p className="font-mono text-xs text-gray-500">
                                {mAsset.assetId} (Detail lengkap tidak
                                ditemukan)
                              </p>
                            </td>
                          </tr>
                      ...
                      return (
                        <tr
                          key={fullAsset.id}
                          className={`transition-colors hover:bg-gray-50 ${color ? `${color.bg} border-l-4 ${color.border}` : ""}`}
                        >
                          <td className="p-3 text-center font-semibold text-gray-800">
                            {index + 1}.
                          </td>
                          ...
                          <td className="p-3 font-mono text-gray-600">
                            {fullAsset.serialNumber || "-"}
                          </td>
                          <td className="p-3 font-mono text-gray-600">
                            {fullAsset.macAddress || "-"}
                          </td>
                          <td className="p-3 font-semibold text-gray-800">
                            {fullAsset.condition}
                          </td>
                      ...
                      return (
                        <tr key={rep.newAssetId || index}>
                          ...
                          <td
                            ...
                            colSpan={5}
                          >
                            Data Aset Pengganti tidak ditemukan (ID:{" "}
                            {rep.newAssetId})
                          </td>
                        </tr>
                      );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3 font-mono text-gray-600">
                          <ClickableLink
                            onClick={() =>
                              onShowPreview({ type: "asset", id: newAsset.id })
                            }
                          >
                            {newAsset.id}
                          </ClickableLink>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="p-3 font-semibold text-center text-gray-600">
                    Jumlah
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {maintenance.materialsUsed &&
                maintenance.materialsUsed.length > 0 ? (
                  maintenance.materialsUsed.map((material, index) => {
                    const materialAsset = material.materialAssetId
                      ? assets.find((a) => a.id === material.materialAssetId)
                      : null;
                    ...
                    if (!materialAsset) {
                      return (
                        <tr
                          key={index}
                          className="hover:bg-gray-50 bg-red-50/50"
                        >
                          <td className="p-3 text-center font-semibold text-gray-800">
                            {index + 1}.
                          </td>
                          ...
                          <td
                            className="p-3 text-red-600 italic font-mono"
                            colSpan={3}
                          >
          ...
          <div className="grid grid-cols-2 text-center text-sm">
            <div>
              ...
              <div className="flex items-center justify-center mt-2 h-28">
                <SignatureStamp
                  signerName={maintenance.technician}
                  signatureDate={maintenance.maintenanceDate}
                />
              </div>
            ...
            <div>
              ...
              <div className="flex items-center justify-center mt-2 h-28">
                {maintenance.status === ItemStatus.COMPLETED &&
                maintenance.completedBy ? (
                  <ApprovalStamp
                    approverName={maintenance.completedBy}
                    approvalDate={maintenance.completionDate!}
                  />
                ) : (
