...
const MaintenanceForm: React.FC<MaintenanceFormProps> = ({
  ...
  const [replacements, setReplacements] = useState<
    Record<string, Partial<MaintenanceReplacement>>
  >({});
  ...
  const assetsForCustomer = useMemo(
    () => getCustomerAssets(selectedCustomerId),
    [selectedCustomerId, getCustomerAssets],
  );
  ...
  const availableSuggestions = useMemo(() => {
    return allWorkTypes.filter(
      (wt) =>
        ...
        wt.toLowerCase().includes(workTypeInput.toLowerCase()),
    );
  ...
  const technicianOptions = useMemo(
    () =>
      users
        .filter((u) => u.divisionId === 3)
        .map((u) => ({ value: u.name, label: u.name })),
    [users],
  );

  const handleWorkTypeInputChange = (
  ...
  const toggleReplacement = (assetId: string) => {
    setReplacements((prev) => {
      ...
      if (newReplacements[assetId]) {
        ...
        newReplacements[assetId] = {
          oldAssetId: assetId,
          retrievedAssetCondition: AssetCondition.USED_OKAY,
        };
      }
      return newReplacements;
    });
  ...
  const updateReplacementDetail = (
    oldAssetId: string,
    field: keyof MaintenanceReplacement,
    value: any,
  ) => {
    setReplacements((prev) => ({
      ...prev,
  ...
  const addAdditionalMaterial = () => {
    setAdditionalMaterials((prev) => [
      ...prev,
      { id: Date.now(), modelKey: "", quantity: 1, unit: "Pcs" },
    ]);
  };

  const handleMaintainMaterial = (installed: {
    itemName: string;
    brand: string;
    unit: string;
    quantity: number;
    ...
    if (exists) {
      addNotification(
        "Material ini sudah ada dalam daftar maintenance.",
        "info",
      );
      return;
    }
  ...
  const handleMaterialChange = (
    id: number,
    field: keyof AdditionalMaterialItem,
    value: any,
  ) => {
    setAdditionalMaterials((prev) =>
      prev.map((item) => {
      ...
      setAdditionalMaterials((prev) =>
        prev.map((item, idx) => {
          if (idx === allocationModal.itemIndex) {
            ...
            const isMeasurement =
              asset.initialBalance !== undefined &&
            ...
            if (isMeasurement) {
              ...
              if (Number(item.quantity) > currentBalance) {
                ...
                addNotification(
                  `Kuantitas disesuaikan dengan sisa stok aset (${newQuantity}).`,
                  "info",
                );
              }
            } else {
              ...
              if (item.quantity !== 1) {
                addNotification(
                  "Aset individual terpilih, kuantitas diatur ke 1.",
                  "info",
                );
              }
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasNewMaterials =
      additionalMaterials.filter((m) => m.modelKey && m.quantity).length > 0;

    if (!customer || (!hasAssets && !hasNewMaterials)) {
      addNotification(
        "Pilih setidaknya satu aset atau tambahkan material untuk membuat laporan.",
        "error",
      );
      return;
    }
    ...
    const finalReplacements = (
      Object.values(replacements) as Partial<MaintenanceReplacement>[]
    ).filter((r): r is MaintenanceReplacement => {
      return !!(r && r.oldAssetId && r.newAssetId && r.retrievedAssetCondition);
    });

    const finalWorkTypes =
      finalReplacements.length > 0
        ? [...new Set([...workTypes, "Ganti Perangkat"])]
        : workTypes;
    ...
    onSave({
      ...
      materialsUsed:
        finalMaterialsUsed.length > 0 ? finalMaterialsUsed : undefined,
      replacements:
        finalReplacements.length > 0 ? finalReplacements : undefined,
      notes: notes.trim(),
    });
  };
  ...
  const ActionButtons: React.FC<{ formId: string }> = ({ formId }) => (
    <>
      ...
      <button
        ...
        form={formId}
        disabled={isLoading}
        className="inline-flex items-center justify-center px-6 py-2.5 text-sm font-semibold text-white transition-all duration-200 rounded-lg shadow-sm bg-primary-600 hover:bg-primary-700 disabled:bg-primary-600/70"
      >
      ...
      <form id={formId} onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                value={docNumber}
                readOnly
                className="w-full mt-1 p-2 bg-gray-100 border border-gray-200 rounded-md text-gray-600"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">
                Nomor Request Terkait
              </label>
              <input
                type="text"
                value={requestNumber}
                ...
                placeholder="Opsional, cth: REQ-001"
              />
            </div>
          </div>
        </section>
        ...
        <section>
          <h4 className="font-semibold text-gray-800 border-b pb-1 mb-4 flex items-center gap-2">
            <BsBoxSeam className="text-primary-600" /> Perangkat Terpasang
            (Pengecekan)
          </h4>
            ...
            <table className="min-w-full text-sm divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="w-40 px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">
                    Aksi
                  </th>
                </tr>
              </thead>
                ...
                {assetsForCustomer.length > 0 ? (
                  assetsForCustomer.map((asset) => {
                    ...
                    const otherSelected = (
                      Object.values(
                        replacements,
                      ) as Partial<MaintenanceReplacement>[]
                    )
                      ...
                      <React.Fragment key={asset.id}>
                        ...
                        >
                          ...
                          <td className="px-4 py-3 font-mono text-xs text-gray-500 align-top">
                            {asset.id} <br /> SN: {asset.serialNumber || "-"}
                          </td>
                          <td className="px-4 py-3 text-center align-top">
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                toggleReplacement(asset.id);
                              }}
                              disabled={!isSelected}
                              className={`px-3 py-1.5 text-xs font-semibold text-white rounded-md shadow-sm transition-colors ${isReplacingThis ? "bg-red-500 hover:bg-red-600" : "bg-primary-500 hover:bg-primary-600"} disabled:bg-gray-300 disabled:cursor-not-allowed`}
                            >
                              {isReplacingThis
                                ? "Batal Ganti"
                                : "Ganti Perangkat"}
                          ...
                          <tr className="bg-blue-50/30">
                            <td colSpan={4} className="p-4">
                              <div className="p-4 bg-white border border-blue-200 rounded-lg shadow-inner space-y-4">
                                <h5 className="text-sm font-bold text-primary-600 flex items-center gap-2">
                                  <BsWrench /> Panel Penggantian Perangkat
                                </h5>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <CustomSelect
                                      ...
                                      value={
                                        replacements[asset.id]
                                          ?.retrievedAssetCondition || ""
                                      }
                                      onChange={(value) =>
                                        updateReplacementDetail(
                                          asset.id,
                                          "retrievedAssetCondition",
                                          value,
                                        )
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-gray-700 mb-1">
                                      Aset Pengganti (Dari Stok)
                                    </label>
                                    <CustomSelect
                                      options={getReplacementOptions(
                                        asset.id,
                                        otherSelected,
                                      )}
                                      value={
                                        replacements[asset.id]?.newAssetId || ""
                                      }
                                      onChange={(value) =>
                                        updateReplacementDetail(
                                          asset.id,
        ...
        <section>
          <h4 className="font-semibold text-gray-800 border-b pb-1 mb-4 flex items-center gap-2">
            <BsLightningFill className="text-orange-600" /> Material Terpasang
            (Infrastruktur)
          </h4>
            ...
            <table className="min-w-full text-sm divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="w-32 px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">
                    Aksi
                  </th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {selectedCustomer?.installedMaterials &&
                selectedCustomer.installedMaterials.length > 0 ? (
                  selectedCustomer.installedMaterials.map((mat, idx) => {
                    const isAlreadyAdded = additionalMaterials.some(
                      (m) => m.modelKey === `${mat.itemName}|${mat.brand}`,
                    );
                    return (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            ...
                            className={`px-3 py-1.5 text-xs font-semibold rounded-md shadow-sm transition-colors border flex items-center justify-center gap-1 mx-auto ${isAlreadyAdded ? "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed" : "bg-white text-orange-600 border-orange-200 hover:bg-orange-50"}`}
                          >
                            {isAlreadyAdded ? (
                              "Ditambahkan"
                            ) : (
                  ...
                  <tr>
                    <td
                      colSpan={3}
                      className="px-6 py-8 text-center text-gray-500 bg-gray-50/50 italic"
                    >
                      Belum ada material infrastruktur yang terdata pada
                      pelanggan ini.
                    </td>
                  </tr>
                )}
        ...
        <section>
          <h4 className="font-semibold text-gray-800 border-b pb-1 mb-4 flex items-center gap-2">
            <BsWrench className="text-gray-500" /> Material Digunakan /
            Sparepart (Input)
          </h4>
          <div className="border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <table className="min-w-full text-sm divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase w-5/12">
                    Material Name
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase w-2/12">
                    Qty Digunakan
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase w-4/12">
                    Sumber Stok
                  </th>
                    ...
                    <tr key={material.id} className="bg-white">
                      <td className="px-4 py-3 align-top">
                        <div className="flex flex-col gap-1">
                          <CustomSelect
                            ...
                            onChange={(val) =>
                              handleMaterialChange(material.id, "modelKey", val)
                            }
                            placeholder="Pilih material..."
                            isSearchable
                          />
                          {material.materialAssetId && (
                            <span className="text-[10px] text-blue-600 font-mono bg-blue-50 px-1 rounded w-fit">
                              Sumber: {material.materialAssetId}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 align-top">
                        <button
                          type="button"
                          onClick={() =>
                            handleOpenAllocationModal(index, material.modelKey)
                          }
                      ...
                      <td className="px-4 py-3 text-center align-top">
                        <button
                          ...
                          className="p-2 text-red-500 rounded-full hover:bg-red-100 bg-white border border-gray-200"
                        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
              ...
              <div className="relative">
                <div className="flex flex-wrap items-center gap-2 p-2 border border-gray-300 rounded-lg min-h-[42px] bg-gray-50">
                  {workTypes.map((workType) => (
                    <span
                      key={workType}
                      className="inline-flex items-center gap-2 px-2.5 py-1 text-sm font-medium text-white bg-primary-600 rounded-full"
                    >
                      {workType}
                      <button
                        type="button"
                        onClick={() => removeWorkType(workType)}
                        className="p-0.5 -mr-1 text-white/70 rounded-full hover:bg-white/20"
                      >
                  ...
                  <input
                    ...
                    placeholder={
                      workTypes.length === 0
                        ? "Ketik lingkup pekerjaan, lalu Enter..."
                        : ""
                    }
            ...
            <div>
              ...
              <CustomSelect
                ...
                value={priority}
                onChange={(value) =>
                  setPriority(value as "Tinggi" | "Sedang" | "Rendah")
                }
              />
            </div>
          </div>
        ...
        <section>
          <div>
            <label className="block text-sm font-medium text-gray-700">
              Laporan Masalah & Diagnosa
            </label>
            <textarea
              value={problemDescription}
              onChange={(e) => setProblemDescription(e.target.value)}
              rows={3}
              className="block w-full px-3 py-2 mt-1 text-gray-900 placeholder:text-gray-400 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
              required
              placeholder="Jelaskan keluhan pelanggan dan hasil diagnosa teknisi."
            />
          </div>
          <div className="mt-4">
            <label className="block text-sm font-medium text-gray-700">
          ...
          <div className="mt-4">
            ...
            <textarea
              ...
              rows={2}
              className="block w-full px-3 py-2 mt-1 text-gray-900 placeholder:text-gray-400 bg-gray-50 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
              placeholder="Catatan lain yang tidak termasuk di atas..."
            />
          </div>
        </section>
        ...
        <section>
          <div className="mt-4">
            ...
            <div
              ...
              onDrop={handleDrop}
              className={`flex items-center justify-center w-full px-6 pt-5 pb-6 mt-1 border-2 border-dashed rounded-md transition-colors ${isDragging ? "border-primary-600 bg-blue-50" : "border-gray-300"}`}
            >
              <div className="space-y-1 text-center">
                ...
                <div className="flex text-sm text-gray-600">
                  <label
                    htmlFor="file-upload"
                    className="relative font-medium bg-transparent rounded-md cursor-pointer text-primary-600 hover:text-primary-500 focus-within:outline-none"
                  >
                ...
                <p className="text-xs text-gray-500">
                  PNG, JPG, PDF hingga {MAX_FILE_SIZE_MB}MB
                </p>
              </div>
            </div>
            ...
            {files.length > 0 && (
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                {files.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between p-2 text-sm text-gray-700 bg-white border border-gray-200 rounded-md shadow-sm"
                  >
