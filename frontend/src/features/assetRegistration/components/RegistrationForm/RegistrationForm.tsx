...
const ActionButtons: React.FC<{ formId?: string, onBack: () => void, isSubmitting: boolean, isEditing: boolean }> = ({ formId, onBack, isSubmitting, isEditing }) => (
    <>
...
export const RegistrationForm: React.FC<RegistrationFormProps> = ({ onBack, onSave, prefillData, editingAsset, currentUser, onStartScan, bulkItems, setBulkItems, assetCategories, setActivePage, openModelModal, openTypeModal }) => {
    // STATE
    ...
    return (
        <>
            <form id={formId} className="space-y-8" onSubmit={handleSubmit}>
                ...
                >
                  <ActionButtons formId={formId} onBack={onBack} isSubmitting={isSubmitting} isEditing={isEditing} />
                </div>
            </form>
