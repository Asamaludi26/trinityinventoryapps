...
export const FormSection: React.FC<{
  ...
  <div
    className={`pt-6 border-t border-gray-200 first:pt-0 first:border-t-0 ${className}`}
  >
...
export const IdentitySection: React.FC<IdentitySectionProps> = ({
  ...
  <FormSection
    ...
    icon={<InfoIcon className="w-6 h-6 mr-3 text-primary-600" />}
  >
      ...
      <div>
        ...
        <div className="mt-1">
          <CustomSelect
            ...
            placeholder={
              formData.categoryId ? "-- Pilih Tipe --" : "Pilih kategori dahulu"
            }
      ...
      <div>
        ...
        <div className="mt-1">
          <CustomSelect
            ...
            placeholder={
              formData.typeId ? "-- Pilih Model --" : "Pilih tipe dahulu"
            }
    ...
    <div className="md:col-span-2">
      <label className="block text-sm font-medium text-gray-700">
        Nama Aset (Otomatis / Custom)
      </label>
      <input
        type="text"
        value={formData.assetName}
        onChange={(e) => updateField("assetName", e.target.value)}
        className="block w-full px-3 py-2 mt-1 text-gray-900 bg-white border border-gray-300 rounded-md shadow-sm sm:text-sm focus:ring-primary-600 focus:border-primary-600"
...
export const FinancialSection: React.FC<FinancialSectionProps> = ({
  ...
  const totalPrice =
    (Number(formData.purchasePrice) || 0) * (Number(formData.quantity) || 0);

  return (
    <FormSection
      title="Informasi Pembelian"
      icon={<DollarIcon className="w-6 h-6 mr-3 text-primary-600" />}
    >
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 md:col-span-2">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Dihitung dari {formData.quantity || 0} unit.
          </p>
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <DatePicker
          ...
          onDateChange={(d) =>
            updateField("purchaseDate", d?.toISOString().split("T")[0])
          }
          disableFutureDates
          disabled={disabled}
        />
...
export const TrackingSection: React.FC<TrackingSectionProps> = ({
  ...
  const modelUnit =
    selectedModel?.unitOfMeasure || selectedType?.unitOfMeasure || "Unit";
  ...
  const totalMeasurementLength = formData.bulkItems.reduce(
    (acc, item) => acc + (item.initialBalance || 0),
    0,
  );
  ...
  const isCountType =
    selectedType?.trackingMethod === "bulk" && !isMeasurementType;
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-amber-800">
                  <p className="font-semibold">
                    Mengedit Aset Individual (Tipe Material)
                  </p>
                  <p>
                    Anda sedang mengedit entitas tunggal dari tipe aset yang
                    biasanya dicatat secara massal.
                  </p>
                </div>
              </div>
            </div>
            ...
            <>
              <div className="flex items-center justify-between mb-2">
                ...
                {!isEditing && (
                  <button
                    type="button"
                    onClick={addBulkItem}
                    className="px-3 py-1 text-xs font-semibold text-white transition-colors duration-200 rounded-md shadow-sm bg-primary-500 hover:bg-primary-600"
                  >
                    + Tambah {unitLabel}
                  </button>
                )}
              ...
              {isCountType && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-lg text-xs text-blue-700 flex items-center gap-2">
                  ...
                  <span>
                    Item ini adalah <strong>Material Massal (Count)</strong>.
                    Sistem akan menghasilkan ID unik untuk setiap baris, tetapi
                    Nomor Seri & MAC Address tidak diperlukan.
                  </span>
                </div>
              )}
              ...
              <div className="space-y-3">
                {formData.bulkItems.map((item, index) => (
                  ...
                  >
                      ...
                      <>
                        ...
                        <div className="md:col-span-1 flex items-end justify-start md:justify-center">
                          <button
                            ...
                            className="flex items-center justify-center w-full h-10 px-3 text-gray-600 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-100 hover:text-primary-600"
                            title="Pindai SN/MAC"
                          >
                    ...
                    {formData.bulkItems.length > 1 && !isEditing && (
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="p-4 border-l-4 rounded-r-lg bg-indigo-50 border-indigo-400">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-indigo-800">
                    <p className="font-semibold">
                      Mode Pencatatan Terukur (Measurement)
                    </p>
                    <p>
                      Aset akan dicatat sebagai unit fisik (Contoh: Drum/Roll)
                      dengan saldo isi (Contoh: Meter) yang dapat berkurang saat
                      digunakan.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                  <div>
                    ...
                    <input
                      ...
                      value={genQty}
                      onChange={(e) =>
                        setGenQty(Math.max(1, parseInt(e.target.value) || 0))
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={genLength}
                      onChange={(e) =>
                        setGenLength(Math.max(1, parseInt(e.target.value) || 0))
                      }
                      className="block w-full px-3 py-2 text-gray-900 bg-white border border-gray-300 rounded-md shadow-sm sm:text-sm"
                    />
                  </div>
                  <button
                    ...
                    onClick={() =>
                      generateMeasurementItems &&
                      generateMeasurementItems(genQty, genLength)
                    }
              ...
              <div className="space-y-3">
                {formData.bulkItems.map((item, index) => (
                  <div
                    key={item.id}
                    className="relative grid grid-cols-12 gap-3 p-3 bg-white border border-gray-200 rounded-lg items-end"
                  >
                    ...
                    <div className="col-span-12 sm:col-span-5">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => onStartScan(item.id)}
                          className="p-1.5 text-gray-500 bg-gray-100 rounded hover:text-primary-600 border border-gray-300"
                        >
                ...
                <div className="flex justify-between items-center p-4 bg-indigo-50 border border-indigo-100 rounded-lg">
                  ...
                  <div className="text-right">
                    <span className="block text-lg font-bold text-indigo-900">
                      {formData.bulkItems.length} {unitLabel}
                    </span>
                    <span className="block text-xs text-indigo-600 font-medium">
                      Total Isi:{" "}
                      {totalMeasurementLength.toLocaleString("id-ID")}{" "}
                      {baseUnit}
                    </span>
                  </div>
                </div>
              ...
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={() => addBulkItem()}
                  className="text-xs flex items-center gap-1 font-semibold text-indigo-600 hover:text-indigo-800"
                >
...
const locationOptions = assetLocations.map((loc) => ({
  value: loc,
  label: loc,
}));
...
export const ContextSection: React.FC<SectionProps> = ({
  ...
  <FormSection
    ...
    icon={<WrenchIcon className="w-6 h-6 mr-3 text-primary-600" />}
  >
    <div>
      <label className="block text-sm font-medium text-gray-700">
        Kondisi Aset
    ...
    <div>
      ...
      <input
        ...
        onChange={(e) => updateField("currentUser", e.target.value)}
        className="block w-full px-3 py-2 mt-1 text-gray-900 bg-gray-50 border border-gray-300 rounded-md shadow-sm sm:text-sm"
      />
    </div>
    <div className="md:col-span-2">
...
export const AttachmentSection: React.FC<SectionProps> = ({
  formData,
  updateField,
}) => {
  ...
  return (
    <FormSection
      title="Lampiran"
      icon={<PaperclipIcon className="w-6 h-6 mr-3 text-primary-600" />}
    >
      <div className="md:col-span-2">
        <label className="block text-sm font-medium text-gray-700">
