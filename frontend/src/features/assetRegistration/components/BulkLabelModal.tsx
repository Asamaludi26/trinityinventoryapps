...
export const BulkLabelModal: React.FC<BulkLabelModalProps> = ({
  isOpen,
  onClose,
  assets,
}) => {
        ...
        <div className="flex justify-between items-center px-1 pb-4 border-b border-gray-200 mb-4 shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Estimasi: {Math.ceil(assets.length / 10)} Halaman (10
              Label/Halaman). Pastikan margin printer diatur ke 'Default' atau
              'Minimum'.
            </p>
          </div>
          <div className="flex gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-bold text-white bg-primary-600 rounded-xl shadow-lg hover:bg-primary-700 active:scale-95 transition-all disabled:bg-primary-600/70"
            >
              {isPreparing ? (
        ...
        <div className="flex-1 overflow-y-auto bg-slate-100 p-8 flex justify-center custom-scrollbar">
          ...
          >
            {assets.map((asset) => (
              <div
                key={asset.id}
                className="asset-label-wrapper flex justify-center items-center"
              >
                {/* Pass 'print-clean' class to force specific print styles */}
                <AssetLabel
                  asset={asset}
                  className="transform scale-[0.95] print-clean"
                />
              </div>
            ))}
          </div>
