...
import {
  ...
  getHandoverInitialState,
} from "./logic/handoverStrategies";
import {
  BsBriefcase,
  BsArchive,
...
export const HandoverForm: React.FC<HandoverFormProps> = ({
  ...
  const [woRoIntNumber, setWoRoIntNumber] = useState(
    initialData?.woRoIntNumber || "",
  );
  const [targetStatus, setTargetStatus] = useState<AssetStatus>(
    initialData?.targetAssetStatus || AssetStatus.IN_USE,
  );

  // SNAPSHOT SECURITY
  const [initialItemsSnapshot] = useState<HandoverItem[]>(
    initialData?.items || [],
  );
  ...
  const filteredUserOptions = useMemo(() => {
    ...
    const userList = users.filter(
      (u) => u.divisionId?.toString() === selectedDivisionId,
    );
  ...
  const handleAssetSelection = (id: number, selectedAssetId: string) => {
    ...
    const typeConfig = catConfig?.types.find(
      (t) => t.name === selectedAsset.type,
    );
    const modelConfig = typeConfig?.standardItems?.find(
      (m) => m.name === selectedAsset.name && m.brand === selectedAsset.brand,
    );
    ...
    setItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === id) {
          ...
          const isTargetingContainer =
            currentUnit === containerUnit && isMeasurement;

          // Auto-switch unit logic
          let targetUnit = currentUnit;
          if (!targetUnit || targetUnit === "Unit") {
            targetUnit = isMeasurement
              ? containerUnit
              : isBulkCount
                ? baseUnit
                : "Unit";
  ...
  const handleItemChange = (
    ...
    field: keyof Omit<
      HandoverItem,
      "id" | "itemName" | "itemTypeBrand" | "assetId"
    >,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isSystemInitiated) {
      ...
      for (const item of items) {
        let matchIndex = snapshotPool.findIndex(
          (snap) =>
        ...
        if (matchIndex === -1) {
          matchIndex = snapshotPool.findIndex(
            (snap) =>
              snap.itemName === item.itemName &&
              snap.itemTypeBrand === item.itemTypeBrand,
          );
        ...
        if (asset && asset.currentBalance !== undefined) {
          if (item.quantity > asset.currentBalance + 0.0001) {
            addNotification(
              `Jumlah "${item.itemName}" melebihi sisa stok fisik aset terpilih (${asset.currentBalance}).`,
              "error",
            );
            return;
          }
  ...
  const isDismantleFlow =
    initialData?.targetAssetStatus === AssetStatus.IN_STORAGE;

  // --- REFACTORED: Row Options Logic (Enhanced for Bulk/Count) ---
  const getRowOptions = useCallback(
    (currentItem: HandoverItem) => {
      ...
      if (currentItem.itemName) {
        candidates = candidates.filter(
          (a) =>
            ...
            (currentItem.itemTypeBrand
              ? a.brand.toLowerCase() ===
                currentItem.itemTypeBrand.toLowerCase()
              : true),
        );
      }
  ...
  const handleKeyDownIntegerOnly = (
  ...
  return (
    <>
      <form id={formId} onSubmit={handleSubmit} className="space-y-6">
        ...
        {initialData?.notes && (
          <div className="p-4 mb-4 border-l-4 rounded-r-lg bg-blue-50 border-primary-600">
            <div className="flex items-start gap-3">
              <InfoIcon className="flex-shrink-0 w-5 h-5 mt-0.5 text-primary-600" />
        ...
        <div className="p-4 border-t border-b border-gray-200">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            <div>
              ...
              <DatePicker
                id="handoverDate"
                selectedDate={handoverDate}
                onDateChange={setHandoverDate}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">
            ...
            <div>
              ...
              <input
                type="text"
                value={woRoIntNumber}
                onChange={(e) => setWoRoIntNumber(e.target.value)}
            ...
            <div>
              ...
              <CustomSelect
                ...
                disabled={!!initialData?.divisionId}
              />
            </div>
            <div className="md:col-span-2">
              ...
              <CustomSelect
                ...
                disabled={!selectedDivisionId || !!initialData?.penerima}
              />
            </div>
          </div>
        </div>
        ...
        <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          {isDismantleFlow ? (
            <div className="p-3 bg-amber-50 text-amber-800 rounded border border-amber-200 flex gap-2 items-center text-sm">
              ...
              <span>
                Status dikunci ke <strong>Di Gudang</strong> karena proses
                Penarikan Aset.
              </span>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label
                className={`relative flex items-start p-4 cursor-pointer rounded-lg border-2 transition-all ${targetStatus === AssetStatus.IN_USE ? "border-blue-500 bg-blue-50" : "border-gray-200 bg-white hover:bg-gray-50"}`}
              >
                <input
                  type="radio"
                  name="targetStatus"
                  value={AssetStatus.IN_USE}
                  checked={targetStatus === AssetStatus.IN_USE}
                  onChange={() => setTargetStatus(AssetStatus.IN_USE)}
                  className="mt-1 h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                />
                <div className="ml-3">
                  <span className="block text-sm font-bold text-gray-900 flex items-center gap-2">
                    <BsBriefcase className="text-blue-500" /> Langsung Digunakan
                  </span>
                  <span className="block text-xs text-gray-500 mt-1">
                    Aset langsung dipakai bekerja oleh penerima (Laptop, Tools).
                  </span>
                </div>
              </label>
              <label
                className={`relative flex items-start p-4 cursor-pointer rounded-lg border-2 transition-all ${targetStatus === AssetStatus.IN_CUSTODY ? "border-purple-500 bg-purple-50" : "border-gray-200 bg-white hover:bg-gray-50"}`}
              >
                <input
                  type="radio"
                  name="targetStatus"
                  value={AssetStatus.IN_CUSTODY}
                  checked={targetStatus === AssetStatus.IN_CUSTODY}
                  onChange={() => setTargetStatus(AssetStatus.IN_CUSTODY)}
                  className="mt-1 h-4 w-4 text-purple-600 border-gray-300 focus:ring-purple-500"
                />
                <div className="ml-3">
                  <span className="block text-sm font-bold text-gray-900 flex items-center gap-2">
                    <BsArchive className="text-purple-500" /> Dipegang /
                    Disimpan (Custody)
                  </span>
                  <span className="block text-xs text-gray-500 mt-1">
                    Aset dipegang penerima untuk stok site/tim, belum aktif
                    dipakai.
                  </span>
                </div>
              </label>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">
              Daftar aset yang diserahterimakan.
            </p>
            {!isSystemInitiated && (
              <button
                type="button"
                onClick={handleAddItem}
                className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-white transition-all duration-200 rounded-lg shadow-sm bg-primary-500 hover:bg-primary-600"
              >
                Tambah Aset
              </button>
            )}
            ...
            {items.map((item, index) => {
              ...
              if (selectedAsset) {
                const catConfig = categories.find(
                  (c) => c.name === selectedAsset.category,
                );
                const typeConfig = catConfig?.types.find(
                  (t) => t.name === selectedAsset.type,
                );
                const modelConfig = typeConfig?.standardItems?.find(
                  (m) =>
                    m.name === selectedAsset.name &&
                    m.brand === selectedAsset.brand,
                );
              ...
              return (
                <div
                  key={item.id}
                  className={`relative p-5 pt-6 bg-white border rounded-xl shadow-sm ${item.isLocked ? "border-l-4 border-l-blue-400 bg-blue-50/20" : "border-gray-200"}`}
                >
                  <div
                    className={`absolute flex items-center justify-center w-8 h-8 font-bold text-white rounded-full -top-4 -left-4 ${item.isLocked ? "bg-blue-600" : "bg-primary-600"}`}
                  >
                    {index + 1}
                  </div>
                  {items.length > 1 &&
                    ...
                    !isSystemInitiated && (
                      <div className="absolute top-2 right-2">
                        <button
                          ...
                          className="flex items-center justify-center w-8 h-8 text-gray-400 transition-colors rounded-full hover:bg-red-100 hover:text-red-500"
                        >
                  ...
                  <div className="grid grid-cols-1 gap-x-6 gap-y-4 md:grid-cols-12">
                    <div className="md:col-span-6">
                      <label className="block text-sm font-bold text-gray-700 flex items-center gap-2 mb-1">
                        {item.isLocked ? (
                          <BsLockFill className="text-blue-600" />
                        ) : (
                          <BsUnlock className="text-gray-400" />
                        )}
                        {isMeasurement
                          ? "Sumber Kabel/Aset"
                          : isCount
                      ...
                      {item.isLocked ? (
                        ...
                        <CustomSelect
                          ...
                          value={item.assetId || ""}
                          onChange={(value) =>
                            handleAssetSelection(item.id, value)
                          }
                          placeholder={
                            rowOptions.length > 0
                              ? "-- Pilih Aset Stok --"
                      ...
                      {selectedAsset && isMeasurement && !item.isLocked && (
                        <div className="mt-1 text-xs text-indigo-600 flex items-center gap-1 font-medium bg-indigo-50 px-2 py-1 rounded w-fit">
                          ...
                          Sisa Stok:{" "}
                          {selectedAsset.currentBalance?.toLocaleString(
                            "id-ID",
                          )}{" "}
                          {baseUnit}
                        </div>
                    ...
                    <div className="md:col-span-3">
                      ...
                      <div className="mt-1 p-2 border border-gray-200 rounded text-sm text-gray-700 h-[38px] truncate bg-gray-100">
                        {item.itemName
                          ? `${item.itemName} ${item.itemTypeBrand ? `(${item.itemTypeBrand})` : ""}`
                          : "-"}
                      </div>
                    </div>
                      ...
                      <div className="flex gap-1 mt-1">
                        ...
                        {isMeasurement ? (
                          <select
                            ...
                            className={`border border-gray-300 text-gray-700 text-xs rounded-r-lg px-2 py-2 font-semibold outline-none focus:ring-2 focus:ring-primary-600 ${isQtyDisabled ? "bg-gray-100 cursor-not-allowed" : "bg-white hover:bg-gray-50 cursor-pointer"}`}
                            disabled={isQtyDisabled}
                          >
                            <option value={baseUnit}>
                              {baseUnit} (Potong)
                            </option>
                            <option value={containerUnit}>
                              {containerUnit} (Utuh)
                            </option>
                          </select>
                        ) : isCount ? (
                    ...
                    <div className="md:col-span-12">
                      ...
                      <input
                        ...
                        placeholder="Contoh: Baik, lengkap dengan aksesoris"
                      />
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 text-center gap-y-8 md:grid-cols-3 md:gap-x-8">
            <div>
              ...
              <div className="flex items-center justify-center mt-2 h-28">
                {menyerahkan && (
                  <SignatureStamp
                    signerName={menyerahkan}
                    signatureDate={handoverDate?.toISOString() || ""}
                    signerDivision={getDivisionForUser(menyerahkan)}
                  />
                )}
              </div>
              <div className="pt-1 mt-2 border-t border-gray-400">
                <p className="w-full p-1 text-sm text-center text-gray-800 rounded-md">
                  {menyerahkan || "Nama Jelas"}
                </p>
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center justify-center mt-2 h-28">
                {penerima ? (
                  <SignatureStamp
                    signerName={penerima}
                    signatureDate={handoverDate?.toISOString() || ""}
                    signerDivision={getDivisionForUser(penerima)}
                  />
                ) : (
                  <span className="text-sm italic text-gray-400">
                    Pilih penerima di atas
                  </span>
                )}
              </div>
              <div className="pt-1 mt-2 border-t border-gray-400">
            ...
            <div>
              ...
              <div className="flex items-center justify-center mt-2 h-28">
                {mengetahui && (
                  <SignatureStamp
                    signerName={mengetahui}
                    signatureDate={handoverDate?.toISOString() || ""}
                    signerDivision={getDivisionForUser(mengetahui)}
                  />
                )}
              </div>
              <div className="pt-1 mt-2 border-t border-gray-400">
                <p className="w-full p-1 text-sm text-center text-gray-800 rounded-md">
                  {mengetahui || "Nama Jelas"}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2 mr-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50"
          >
            Batal
          </button>
          <button
            ...
            className="inline-flex items-center justify-center px-5 py-2.5 text-sm font-semibold text-white transition-all duration-200 rounded-lg shadow-sm bg-primary-600 hover:bg-primary-700 disabled:bg-primary-600/70 disabled:cursor-not-allowed"
          >
