...
interface HandoverStatusSidebarProps {
  handover: Handover;
  currentUser: User;
  isLoading: boolean;
  isExpanded: boolean;
...
const HandoverStatusIndicator: React.FC<{ status: ItemStatus }> = ({
  status,
}) => {
...
export const HandoverStatusSidebar: React.FC<HandoverStatusSidebarProps> = ({
  handover,
  currentUser,
  isLoading,
  onComplete,
  ...
  const canComplete =
    handover.status === ItemStatus.IN_PROGRESS &&
        ...
        {canComplete ? (
          ...
          <div className="text-center p-4 bg-gray-50/70 border border-gray-200/60 rounded-lg">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Handover ini telah selesai diproses.
            </p>
          </div>
        )}
      </div>
