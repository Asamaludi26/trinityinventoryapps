...
const formatters = {
  ...
  itemsList: (items: RequestItem[]): string => {
    return items
      .map(
        (item, idx) =>
          `${idx + 1}. ${sanitize(item.itemName)} (${sanitize(
            item.itemTypeBrand
          )}) ${item.quantity} Unit`
      )
      .join("\n");
...
export const WhatsAppService = {
  generateNewRequestPayload: (request: Request): WAMessagePayload => {
    const justification = request.order.justification
      ? `📝 *Alasan Urgent:* ${sanitize(request.order.justification)}`
      : "";
    ...
    return {
      ...
      message,
      groupName:
        WA_CONFIG.groupNames[
          WA_CONFIG.groups.LOGISTIC as keyof typeof WA_CONFIG.groupNames
        ],
    };
  },

  generateLogisticApprovalPayload: (
    request: Request,
    approverName: string
  ): WAMessagePayload => {
    const needsPurchase = request.items.some(
      (i) => request.itemStatuses?.[i.id]?.status === "procurement_needed"
    );
    const targetId = needsPurchase
      ? WA_CONFIG.groups.PURCHASE
      : WA_CONFIG.groups.LOGISTIC;
    ...
    const title = hasRevisions
      ? "✅ LOGISTIC APPROVED (DENGAN REVISI)"
      : "✅ LOGISTIC APPROVED";
    ...
    return {
      ...
      message,
      groupName:
        WA_CONFIG.groupNames[targetId as keyof typeof WA_CONFIG.groupNames],
    };
  },

  generateSubmitToCeoPayload: (
    request: Request,
    approverName: string
  ): WAMessagePayload => {
    ...
    return {
      ...
      groupName:
        WA_CONFIG.groupNames[
          WA_CONFIG.groups.MANAGEMENT as keyof typeof WA_CONFIG.groupNames
        ],
    };
  },

  generateFinalApprovalPayload: (
    request: Request,
    approverName: string
  ): WAMessagePayload => {
    ...
    return {
      ...
      groupName:
        WA_CONFIG.groupNames[
          WA_CONFIG.groups.PURCHASE as keyof typeof WA_CONFIG.groupNames
        ],
    };
  },
  ...
  generateProcurementUpdatePayload: (
    request: Request,
    status: ItemStatus
  ): WAMessagePayload => {
    let title = "";
    ...
    if (status === ItemStatus.PURCHASING) {
      ...
      date = request.actualShipmentDate
        ? formatters.date(request.actualShipmentDate)
        : formatters.date(new Date().toISOString());
    }
  ...
  generateItemsArrivedPayload: (request: Request): WAMessagePayload => {
    const arrivalDate = request.arrivalDate
      ? formatters.dateTime(request.arrivalDate)
      : formatters.dateTime(new Date().toISOString());
    const message = buildMessageBody(
      request,
      "🏁 BARANG TIBA / SIAP",
      "Selesai (Barang Tiba)",
  ...
  generateRejectionPayload: (
    ...
    const isCeoRejection =
      rejectorName.includes("Super Admin") || rejectorName.includes("CEO");
    const title = isCeoRejection
      ? "❌ DITOLAK CEO / MANAGEMENT"
      : "❌ REQUEST DITOLAK";
