...
export const MaterialAllocationModal: React.FC<
...
> = ({
  ...
  const [sourceMode, setSourceMode] = useState<"personal" | "warehouse">(
    "personal",
  );

  const isAdmin =
    currentUser.role === "Admin Logistik" || currentUser.role === "Super Admin";
  ...
  const availableStock = useMemo(() => {
    ...
    const filtered = assets
      .filter((a) => {
        ...
        const isItemMatch =
          assetName.includes(targetName) || targetName.includes(assetName);
    ...
    <Modal
      ...
      footerContent={
        <div className="flex justify-between items-center w-full">
          ...
          <button
            onClick={onClose}
            className="px-6 py-2.5 text-sm font-bold text-gray-700 bg-white border border-gray-300 rounded-xl shadow-sm hover:bg-gray-50 transition-all font-sans"
          >
            Batal
          </button>
        </div>
    ...
    >
      <div className="space-y-6">
          ...
          {isAdmin && (
            <div className="flex bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm shrink-0">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-lg transition-all ${sourceMode === "personal" ? "bg-primary-600 text-white shadow-md" : "text-gray-500 hover:bg-gray-50"}`}
              >
                <BsPersonBadge className="w-3.5 h-3.5" /> Stok Teknisi
              </button>
              <button
                onClick={() => setSourceMode("warehouse")}
                className={`flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-lg transition-all ${sourceMode === "warehouse" ? "bg-primary-600 text-white shadow-md" : "text-gray-500 hover:bg-gray-50"}`}
              >
        ...
        <div className="relative group">
          <SearchIcon className="absolute w-5 h-5 text-gray-400 left-4 top-1/2 -translate-y-1/2 group-focus-within:text-primary-600 transition-colors" />
          <input
            type="text"
            placeholder="Cari ID Aset, Serial Number, atau Lokasi..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-12 pr-4 py-3.5 text-sm font-medium border border-gray-200 rounded-xl bg-white shadow-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-600 transition-all outline-none font-sans"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[65vh] overflow-y-auto custom-scrollbar p-1">
          {filteredStock.length > 0 ? (
            filteredStock.map((asset, index) => {
              ...
              const percentage = isMeasurement
                ? Math.min(100, Math.max(0, (balance / initial) * 100))
                : 100;
              ...
              return (
                <div
                  ...
                  className={`
                                        ...
                                        ${
                                          isSelected
                                            ? "border-primary-600 bg-primary-50/60 shadow-md ring-2 ring-primary-500/10"
                                            : isEmpty
                                              ? "border-gray-100 bg-gray-50 opacity-60 cursor-not-allowed"
                                              : "border-gray-200 bg-white hover:border-primary-500/50 hover:shadow-lg cursor-pointer"
                                        }
                                    `}
                >
                  ...
                  <div className="flex items-start gap-4 mb-4">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl shrink-0 ${isSelected ? "bg-primary-600 text-white" : "bg-gray-100 text-gray-500 group-hover:bg-primary-50 group-hover:text-primary-600 transition-colors"}`}
                    >
                      <ArchiveBoxIcon className="w-6 h-6" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="font-display font-bold text-gray-900 text-base truncate">
                          {asset.id}
                        </p>
                        {asset.serialNumber && (
                  ...
                  <div className="flex items-end justify-between pt-3 border-t border-gray-100 mt-auto">
                    <div className="flex-1 mr-4">
                      ...
                      <div className="flex items-baseline gap-1">
                        <p
                          className={`font-display text-2xl font-bold leading-none ${isEmpty ? "text-red-500" : "text-gray-900"}`}
                        >
                          {balance.toLocaleString("id-ID")}
                        </p>
                        <span className="text-xs text-gray-500 font-medium">
                          {isMeasurement ? (
                            <BsRulers className="inline w-3 h-3" />
                          ) : (
                            <BsBoxSeam className="inline w-3 h-3" />
                          )}
                        </span>
                      </div>
                    ...
                    <div className="shrink-0">
                      {isSelected ? (
                        <div className="w-9 h-9 bg-primary-600 text-white rounded-full flex items-center justify-center shadow-lg transform scale-110">
                          <CheckIcon className="w-5 h-5" />
                        </div>
                      ) : (
                        <div
                          className={`w-9 h-9 rounded-full border-2 flex items-center justify-center transition-all ${isEmpty ? "border-gray-200 text-gray-300" : "border-gray-300 text-gray-300 group-hover:border-primary-600 group-hover:text-primary-600 group-hover:bg-primary-50"}`}
                        >
                          <div
                            className={`w-3.5 h-3.5 rounded-full ${isEmpty ? "" : "bg-current opacity-0 group-hover:opacity-100"}`}
                          ></div>
                        </div>
                      )}
                    </div>
            ...
            <div className="col-span-full flex flex-col items-center justify-center py-20 text-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
              ...
              {isAdmin && (
                <button
                  onClick={() =>
                    setSourceMode(
                      sourceMode === "personal" ? "warehouse" : "personal",
                    )
                  }
                  className="mt-6 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-bold text-primary-600 hover:bg-primary-50 transition-colors shadow-sm"
                >
                  Cek di{" "}
                  {sourceMode === "personal" ? "Gudang Utama" : "Stok Teknisi"}?
                </button>
              )}
            </div>
          )}
