...
const NavLink: React.FC<{
  ...
  const baseClasses =
    "relative flex items-center px-4 py-2.5 my-1 rounded-lg text-sm font-medium transition-all duration-200 group";
  ...
  const linkContent = (
    <>
      ...
      {isActive && !item.isExternal && (
        <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-primary-500 rounded-r-full shadow-[0_0_10px_rgba(37,99,235,0.6)]"></span>
      )}
      <item.icon
        className={`flex-shrink-0 w-5 h-5 mr-3 transition-colors ${isActive ? "text-primary-500" : "text-slate-500 group-hover:text-slate-300"}`}
      />
      <span className="tracking-wide">{item.label}</span>
      {item.isExternal && (
        <svg
          className="w-4 h-4 ml-auto text-slate-600"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const SidebarContent = () => (
    // Design Update: Uses Slate-900 (dark theme) with a subtle gradient hint
      ...
      <div className="flex items-center justify-between h-16 px-6 border-b border-slate-800/50 flex-shrink-0 bg-slate-900/50 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <div className="relative">
            <TrinitiLogoIcon className="w-8 h-8 text-primary-500 drop-shadow-[0_0_8px_rgba(37,99,235,0.5)]" />
          </div>
        ...
        <button
          ...
          className="text-slate-400 md:hidden hover:text-white p-1 rounded hover:bg-white/10 transition-colors"
        >
          <CloseIcon />
        </button>
      </div>
            ...
            <div key={item.id} className="mb-1">
              ...
              >
                <div className="flex items-center">
                  <item.icon
                    className={`flex-shrink-0 w-5 h-5 mr-3 transition-colors ${isParentActive ? "text-primary-500" : "text-slate-500 group-hover:text-slate-300"}`}
                  />
                  <span className="tracking-wide">{item.label}</span>
                </div>
                <ChevronDownIcon
                  className={`w-4 h-4 transform transition-transform duration-200 opacity-50 ${openMenus[item.id] ? "rotate-180" : "rotate-0"}`}
                />
              </button>
              <div
                className={`overflow-hidden transition-all duration-300 ease-in-out ${openMenus[item.id] ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}
              >
                    ...
                    return (
                      <div key={child.id} className="mt-1">
                        ...
                        >
                          <div className="flex items-center">
                            <child.icon
                              className={`flex-shrink-0 w-4 h-4 mr-3 transition-colors ${isChildParentActive ? "text-primary-500" : "text-slate-600 group-hover:text-slate-400"}`}
                            />
                            <span>{child.label}</span>
                          </div>
                          <ChevronDownIcon
                            className={`w-3 h-3 transform transition-transform duration-200 ${openMenus[child.id] ? "rotate-180" : "rotate-0"}`}
                          />
                        </button>
                        <div
                          className={`overflow-hidden transition-all duration-300 ease-in-out ${openMenus[child.id] ? "max-h-96" : "max-h-0"}`}
                        >
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-30 h-full w-64 bg-slate-900 transform transition-transform duration-300 ease-in-out md:translate-x-0 no-print shadow-2xl ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <SidebarContent />
      </aside>
    </>
